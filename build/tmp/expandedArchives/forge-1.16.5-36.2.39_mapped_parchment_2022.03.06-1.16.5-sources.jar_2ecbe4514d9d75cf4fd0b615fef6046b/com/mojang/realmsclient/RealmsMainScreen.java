...
public class RealmsMainScreen extends RealmsScreen {
   ...
   public boolean charTyped(char pCodePoint, int pModifiers) {
      this.keyCombos.forEach((p_237578_1_) -> {
         p_237578_1_.keyPressed(pCodePoint);
      });
      return true;
   }
   ...
   public void render(MatrixStack pMatrixStack, int pMouseX, int pMouseY, float pPartialTicks) {
      ...
      if (this.trialsAvailable && !this.createdTrial && this.shouldShowPopup()) {
         ...
         AbstractGui.blit(pMatrixStack, this.createTrialButton.x + this.createTrialButton.getWidth() - 8 - 4, this.createTrialButton.y + this.createTrialButton.getHeight() / 2 - 4, 0.0F, (float)j, 8, 8, 8, 16);
      }
   ...
   class ServerList extends RealmsObjectSelectionList<RealmsMainScreen.ListEntry> {
      ...
      public boolean keyPressed(int pKeyCode, int pScanCode, int pModifiers) {
         if (pKeyCode != 257 && pKeyCode != 32 && pKeyCode != 335) {
            ...
            return extendedlist$abstractlistentry == null ? super.keyPressed(pKeyCode, pScanCode, pModifiers) : extendedlist$abstractlistentry.mouseClicked(0.0D, 0.0D, 0);
         }
      }
         ...
         if (pIndex != -1) {
            ...
            if (this.showingMessage) {
               if (pIndex == 0) {
                  ...
                  if (pIndex - 1 >= RealmsMainScreen.this.realmsServers.size()) {
                     RealmsMainScreen.this.selectedServerId = -1L;
                     return;
                  }

                  realmsserver = RealmsMainScreen.this.realmsServers.get(pIndex - 1);
               }
            } else {
               if (pIndex >= RealmsMainScreen.this.realmsServers.size()) {
                  RealmsMainScreen.this.selectedServerId = -1L;
                  return;
               }

               realmsserver = RealmsMainScreen.this.realmsServers.get(pIndex);
            }
      ...
      public void setSelected(@Nullable RealmsMainScreen.ListEntry pEntry) {
         super.setSelected(pEntry);
         int i = this.children().indexOf(pEntry);
