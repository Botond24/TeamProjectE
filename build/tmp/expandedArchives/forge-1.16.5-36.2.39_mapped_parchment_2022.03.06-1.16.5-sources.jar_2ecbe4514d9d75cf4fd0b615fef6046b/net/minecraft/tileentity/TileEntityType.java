...
public class TileEntityType<T extends TileEntity> extends net.minecraftforge.registries.ForgeRegistryEntry<TileEntityType<?>> {
   ...
   @Nullable
   public static ResourceLocation getKey(TileEntityType<?> pBlockEntityType) {
      return Registry.BLOCK_ENTITY_TYPE.getKey(pBlockEntityType);
   }

   private static <T extends TileEntity> TileEntityType<T> register(String pKey, TileEntityType.Builder<T> pBuilder) {
      if (pBuilder.validBlocks.isEmpty()) {
         LOGGER.warn("Block entity type {} requires at least one valid block to be defined!", (Object)pKey);
      }

      Type<?> type = Util.fetchChoiceType(TypeReferences.BLOCK_ENTITY, pKey);
   ...
   @Nullable
   public T getBlockEntity(IBlockReader pLevel, BlockPos pPos) {
   ...
   public static final class Builder<T extends TileEntity> {
      ...
      public TileEntityType<T> build(Type<?> pDataType) {
         return new TileEntityType<>(this.factory, this.validBlocks, pDataType);
      }
   }
}
