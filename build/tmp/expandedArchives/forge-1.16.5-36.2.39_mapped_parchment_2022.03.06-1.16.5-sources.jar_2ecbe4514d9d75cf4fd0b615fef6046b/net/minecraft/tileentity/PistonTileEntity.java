...
public class PistonTileEntity extends TileEntity implements ITickableTileEntity {
   ...
   public float getProgress(float pPartialTicks) {
      ...
      return MathHelper.lerp(pPartialTicks, this.progressO, this.progress);
   }
   ...
   private static void moveEntityByPiston(Direction pDirection, Entity pEntity, double pProgress, Direction p_227022_4_) {
      NOCLIP.set(pDirection);
      pEntity.move(MoverType.PISTON, new Vector3d(pProgress * (double)p_227022_4_.getStepX(), pProgress * (double)p_227022_4_.getStepY(), pProgress * (double)p_227022_4_.getStepZ()));
   ...
   private static boolean matchesStickyCritera(AxisAlignedBB pShape, Entity pEntity) {
   ...
   private static double getMovement(AxisAlignedBB pHeadShape, Direction pDirection, AxisAlignedBB pFacing) {
      switch(pDirection) {
      case EAST:
   ...
   public CompoundNBT save(CompoundNBT pCompound) {
      ...
      pCompound.put("blockState", NBTUtil.writeBlockState(this.movedState));
   ...
   public VoxelShape getCollisionShape(IBlockReader pLevel, BlockPos pPos) {
      ...
      if (!this.extending && this.isSourcePiston) {
         voxelshape = this.movedState.setValue(PistonBlock.EXTENDED, Boolean.valueOf(true)).getCollisionShape(pLevel, pPos);
      } else {
      ...
      if ((double)this.progress < 1.0D && direction == this.getMovementDirection()) {
         ...
         return VoxelShapes.or(voxelshape, blockstate.getCollisionShape(pLevel, pPos).move(d0, d1, d2));
      }
   }
