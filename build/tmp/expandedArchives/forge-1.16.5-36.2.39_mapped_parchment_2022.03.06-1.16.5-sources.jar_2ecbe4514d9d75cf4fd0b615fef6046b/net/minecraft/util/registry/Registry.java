...
public abstract class Registry<T> implements Codec<T>, Keyable, IObjectIntIterable<T> {
   ...
   private static <T> RegistryKey<Registry<T>> createRegistryKey(String pRegistryName) {
      return RegistryKey.createRegistryKey(new ResourceLocation(pRegistryName));
   }

   public static <T extends MutableRegistry<?>> void checkRegistry(MutableRegistry<T> pMetaRegistry) {
      pMetaRegistry.forEach((p_239739_1_) -> {
         if (p_239739_1_.keySet().isEmpty()) {
            LOGGER.error("Registry '{}' was empty after loading", (Object)pMetaRegistry.getKey(p_239739_1_));
            if (SharedConstants.IS_RUNNING_IN_IDE) {
               throw new IllegalStateException("Registry: '" + pMetaRegistry.getKey(p_239739_1_) + "' is empty, not allowed, fix me!");
            }
         }
    ...
    */
   private static <T> Registry<T> registerSimple(RegistryKey<? extends Registry<T>> pRegistryKey, Supplier<T> pLoader) {
   ...
   private static <T> DefaultedRegistry<T> registerDefaulted(RegistryKey<? extends Registry<T>> pRegistryKey, String pDefaultValueName, Supplier<T> pLoader) {
    ...
    */
   private static <T> Registry<T> registerSimple(RegistryKey<? extends Registry<T>> pRegistryKey, Lifecycle pLifecycle, Supplier<T> pLoader) {
      return internalRegister(pRegistryKey, new SimpleRegistry<>(pRegistryKey, pLifecycle), pLoader, pLifecycle);
   }
   ...
   private static <T> DefaultedRegistry<T> registerDefaulted(RegistryKey<? extends Registry<T>> pRegistryKey, String pDefaultValueName, Lifecycle pLifecycle, Supplier<T> pLoader) {
      return internalRegister(pRegistryKey, new DefaultedRegistry<>(pDefaultValueName, pRegistryKey, pLifecycle), pLoader, pLifecycle);
   }
    ...
    */
   private static <T, R extends MutableRegistry<T>> R internalRegister(RegistryKey<? extends Registry<T>> pRegistryKey, R pRegistry, Supplier<T> pLoader, Lifecycle pLifecycle) {
      ResourceLocation resourcelocation = pRegistryKey.location();
      LOADERS.put(resourcelocation, pLoader);
   ...
   protected Registry(RegistryKey<? extends Registry<T>> pKey, Lifecycle pLifecycle) {
      this.key = pKey;
   ...
   public abstract Optional<RegistryKey<T>> getResourceKey(T pValue);
   ...
   @Nullable
   public abstract T get(@Nullable RegistryKey<T> pKey);
   ...
   public Optional<T> getOptional(@Nullable ResourceLocation pId) {
      return Optional.ofNullable(this.get(pId));
   }

   @OnlyIn(Dist.CLIENT)
   public Optional<T> getOptional(@Nullable RegistryKey<T> pRegistryKey) {
      return Optional.ofNullable(this.get(pRegistryKey));
   }

   public T getOrThrow(RegistryKey<T> pKey) {
      T t = this.get(pKey);
      if (t == null) {
   ...
   public static <V, T extends V> T register(Registry<V> pRegistry, ResourceLocation pName, T pValue) {
      return ((MutableRegistry<V>)pRegistry).register(RegistryKey.create(pRegistry.key, pName), pValue, Lifecycle.stable());
   }

   public static <V, T extends V> T registerMapping(Registry<V> pRegistry, int pId, String pName, T pValue) {
      return ((MutableRegistry<V>)pRegistry).registerMapping(pId, RegistryKey.create(pRegistry.key, new ResourceLocation(pName)), pValue, Lifecycle.stable());
   }
