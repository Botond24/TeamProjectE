...
public class UpgradeData {
   ...
   public void upgrade(Chunk pChunk) {
   ...
   private static void upgradeSides(Chunk pChunk, Direction8 p_196991_1_) {
      World world = pChunk.getLevel();
      if (pChunk.getUpgradeData().sides.remove(p_196991_1_)) {
   ...
   private static BlockState updateState(BlockState pState, Direction pDirection, IWorld pLevel, BlockPos pPos, BlockPos pOffsetPos) {
   ...
   private void upgradeInside(Chunk pChunk) {
      BlockPos.Mutable blockpos$mutable = new BlockPos.Mutable();
      BlockPos.Mutable blockpos$mutable1 = new BlockPos.Mutable();
      ChunkPos chunkpos = pChunk.getPos();
      IWorld iworld = pChunk.getLevel();

      for(int i = 0; i < 16; ++i) {
         ChunkSection chunksection = pChunk.getSections()[i];
   ...
   static enum BlockFixers implements UpgradeData.IBlockFixer {
      ...
      LEAVES(true, Blocks.ACACIA_LEAVES, Blocks.BIRCH_LEAVES, Blocks.DARK_OAK_LEAVES, Blocks.JUNGLE_LEAVES, Blocks.OAK_LEAVES, Blocks.SPRUCE_LEAVES) {
         ...
         public void processChunk(IWorld pLevel) {
            BlockPos.Mutable blockpos$mutable = new BlockPos.Mutable();
            List<ObjectSet<BlockPos>> list = this.queue.get();
            ...
            for(int i = 2; i < list.size(); ++i) {
               ...
               for(BlockPos blockpos : objectset) {
                  ...
                  if (blockstate.getValue(BlockStateProperties.DISTANCE) >= j) {
                     pLevel.setBlock(blockpos, blockstate.setValue(BlockStateProperties.DISTANCE, Integer.valueOf(j)), 18);
                     if (i != 7) {
      ...
      private BlockFixers(Block... pBlocks) {
         this(false, pBlocks);
      }

      private BlockFixers(boolean p_i49366_3_, Block... pBlocks) {
   ...
   public interface IBlockFixer {
      ...
      default void processChunk(IWorld pLevel) {
      }
   }
}