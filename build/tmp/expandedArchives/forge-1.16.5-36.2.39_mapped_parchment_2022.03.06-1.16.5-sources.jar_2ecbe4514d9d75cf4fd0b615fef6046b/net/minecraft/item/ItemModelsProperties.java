...
public class ItemModelsProperties {
   ...
   private static IItemPropertyGetter registerGeneric(ResourceLocation pName, IItemPropertyGetter pProperty) {
      GENERIC_PROPERTIES.put(pName, pProperty);
      return pProperty;
   }

   /**
    * Registers an {@link net.minecraft.client.renderer.item.ItemPropertyFunction} (used by models) to the item.
    */
   public static void register(Item pItem, ResourceLocation pName, IItemPropertyGetter pProperty) {
      PROPERTIES.computeIfAbsent(pItem, (p_239416_0_) -> {
         return Maps.newHashMap();
      }).put(pName, pProperty);
   }

   @Nullable
   public static IItemPropertyGetter getProperty(Item pItem, ResourceLocation pName) {
      ...
      IItemPropertyGetter iitempropertygetter = GENERIC_PROPERTIES.get(pName);
      if (iitempropertygetter != null) {
         return iitempropertygetter;
      } else {
         Map<ResourceLocation, IItemPropertyGetter> map = PROPERTIES.get(pItem);
   ...
   static class Angle {
      ...
      private void update(long pLastUpdateTick, double p_239449_3_) {
         this.lastUpdateTick = pLastUpdateTick;
