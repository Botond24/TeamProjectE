...
public class CrossbowItem extends ShootableItem implements IVanishable {
   ...
   private static AbstractArrowEntity getArrow(World pLevel, LivingEntity pLivingEntity, ItemStack pCrossbowStack, ItemStack pAmmoStack) {
      ArrowItem arrowitem = (ArrowItem)(pAmmoStack.getItem() instanceof ArrowItem ? pAmmoStack.getItem() : Items.ARROW);
      AbstractArrowEntity abstractarrowentity = arrowitem.createArrow(pLevel, pAmmoStack, pLivingEntity);
   ...
   private static float getShootingPower(ItemStack pCrossbowStack) {
      return pCrossbowStack.getItem() == Items.CROSSBOW && containsChargedProjectile(pCrossbowStack, Items.FIREWORK_ROCKET) ? 1.6F : 3.15F;
   }
