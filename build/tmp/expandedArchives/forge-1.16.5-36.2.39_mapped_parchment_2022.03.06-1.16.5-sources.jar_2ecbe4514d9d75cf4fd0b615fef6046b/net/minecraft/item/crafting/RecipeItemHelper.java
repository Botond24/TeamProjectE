...
public class RecipeItemHelper {
   ...
   private boolean has(int pStackingIndex) {
      return this.contents.get(pStackingIndex) > 0;
   }

   private int take(int pStackingIndex, int pAmount) {
   ...
   private void put(int pStackingIndex, int pIncrement) {
      this.contents.put(pStackingIndex, this.contents.get(pStackingIndex) + pIncrement);
   }

   public boolean canCraft(IRecipe<?> pRecipe, @Nullable IntList pStackingIndexList) {
      return this.canCraft(pRecipe, pStackingIndexList, 1);
   }

   public boolean canCraft(IRecipe<?> pRecipe, @Nullable IntList pStackingIndexList, int pAmount) {
      return (new RecipeItemHelper.RecipePicker(pRecipe)).tryPick(pAmount, pStackingIndexList);
   }

   public int getBiggestCraftableStack(IRecipe<?> pRecipe, @Nullable IntList pStackingIndexList) {
      return this.getBiggestCraftableStack(pRecipe, Integer.MAX_VALUE, pStackingIndexList);
   }

   public int getBiggestCraftableStack(IRecipe<?> pRecipe, int pAmount, @Nullable IntList pStackingIndexList) {
      return (new RecipeItemHelper.RecipePicker(pRecipe)).tryPickAll(pAmount, pStackingIndexList);
   }
   ...
   class RecipePicker {
      ...
      public RecipePicker(IRecipe<?> pRecipe) {
         this.recipe = pRecipe;
      ...
      public boolean tryPick(int pAmount, @Nullable IntList pStackingIndexList) {
         if (pAmount <= 0) {
            ...
            for(i = 0; this.dfs(pAmount); ++i) {
               RecipeItemHelper.this.take(this.items[this.path.getInt(0)], pAmount);
               ...
               if (flag1 && list.get(i1).isEmpty()) {
                  ...
                  for(int j1 = 0; j1 < this.itemCount; ++j1) {
                     if (this.hasResidual(false, l, j1)) {
                        ...
                        RecipeItemHelper.this.put(this.items[j1], pAmount);
                        if (flag1) {
