...
public interface IRecipePlacer<T> {
   default void placeRecipe(int pWidth, int pHeight, int pOutputSlot, IRecipe<?> pRecipe, Iterator<T> pIngredients, int pMaxAmount) {
      int i = pWidth;
      int j = pHeight;
      ...
      for(int k = 0; k < pHeight; ++k) {
         ...
         int l = MathHelper.floor((float)pHeight / 2.0F - (float)j / 2.0F);
         if (flag && l > k) {
            k1 += pWidth;
            ++k;
         }
         ...
         for(int i1 = 0; i1 < pWidth; ++i1) {
            ...
            l = MathHelper.floor((float)pWidth / 2.0F - (float)i / 2.0F);
            int j1 = i;
            boolean flag1 = i1 < i;
            if (flag) {
               j1 = l + i;
