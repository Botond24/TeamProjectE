...
public class Ingredient implements Predicate<ItemStack> {
   ...
   protected Ingredient(Stream<? extends Ingredient.IItemList> pValues) {
      this.values = pValues.toArray((p_209360_0_) -> {
   ...
   public final void toNetwork(PacketBuffer pBuffer) {
      this.dissolve();
      if (!this.isVanilla()) {
         net.minecraftforge.common.crafting.CraftingHelper.write(pBuffer, this);
         return;
      }
      pBuffer.writeVarInt(this.itemStacks.length);

      for(int i = 0; i < this.itemStacks.length; ++i) {
         pBuffer.writeItem(this.itemStacks[i]);
      }
   ...
   public static Ingredient fromValues(Stream<? extends Ingredient.IItemList> pStream) {
      Ingredient ingredient = new Ingredient(pStream);
      return ingredient.values.length == 0 ? EMPTY : ingredient;
   }

   public static Ingredient of(IItemProvider... pItems) {
   ...
   public static Ingredient of(Stream<ItemStack> pStacks) {
      return fromValues(pStacks.filter((p_234824_0_) -> {
   ...
   public static Ingredient of(ITag<Item> pTag) {
      return fromValues(Stream.of(new Ingredient.TagList(pTag)));
   }

   public static Ingredient fromNetwork(PacketBuffer pBuffer) {
      int i = pBuffer.readVarInt();
      if (i == -1) {
         return net.minecraftforge.common.crafting.CraftingHelper.getIngredient(pBuffer.readResourceLocation(), pBuffer);
      }
      return fromValues(Stream.generate(() -> {
         return new Ingredient.SingleItemList(pBuffer.readItem());
      }).limit((long)i));
   }
   ...
   public static Ingredient.IItemList valueFromJson(JsonObject pJson) {
      if (pJson.has("item") && pJson.has("tag")) {
         throw new JsonParseException("An ingredient entry is either a tag or an item, not both");
      } else if (pJson.has("item")) {
         ResourceLocation resourcelocation1 = new ResourceLocation(JSONUtils.getAsString(pJson, "item"));
         ...
         ResourceLocation resourcelocation = new ResourceLocation(JSONUtils.getAsString(pJson, "tag"));
   ...
   public static class SingleItemList implements Ingredient.IItemList {
      ...
      public SingleItemList(ItemStack pItem) {
         this.item = pItem;
      }
