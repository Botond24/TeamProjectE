...
public class BannerItem extends WallOrFloorItem {
   ...
   @OnlyIn(Dist.CLIENT)
   public static void appendHoverTextFromBannerBlockEntityTag(ItemStack pStack, List<ITextComponent> pTooltipComponents) {
      ...
      if (compoundnbt != null && compoundnbt.contains("Patterns")) {
         ...
         for(int i = 0; i < listnbt.size() && i < 6; ++i) {
            ...
            if (bannerpattern != null) {
               pTooltipComponents.add((new TranslationTextComponent("block.minecraft.banner." + bannerpattern.getFilename() + '.' + dyecolor.getName())).withStyle(TextFormatting.GRAY));
            }
         }
