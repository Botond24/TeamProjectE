...
public class ScoreCriteria {
   ...
   public ScoreCriteria(String pName) {
      this(pName, false, ScoreCriteria.RenderType.INTEGER);
   }

   protected ScoreCriteria(String pName, boolean pReadOnly, ScoreCriteria.RenderType pRenderType) {
      this.name = pName;
      this.readOnly = pReadOnly;
      this.renderType = pRenderType;
      CRITERIA_BY_NAME.put(pName, this);
   }

   public static Optional<ScoreCriteria> byName(String pName) {
      if (CRITERIA_BY_NAME.containsKey(pName)) {
         return Optional.of(CRITERIA_BY_NAME.get(pName));
      } else {
         int i = pName.indexOf(58);
         return i < 0 ? Optional.empty() : Registry.STAT_TYPE.getOptional(ResourceLocation.of(pName.substring(0, i), '.')).flatMap((p_216392_2_) -> {
            return getStat(p_216392_2_, ResourceLocation.of(pName.substring(i + 1), '.'));
         });
      }
   }

   private static <T> Optional<ScoreCriteria> getStat(StatType<T> pStatType, ResourceLocation pResourceLocation) {
   ...
   public static enum RenderType {
      ...
      public static ScoreCriteria.RenderType byId(String pRenderType) {
