...
public class ContainerType<T extends Container> extends net.minecraftforge.registries.ForgeRegistryEntry<ContainerType<?>> implements net.minecraftforge.common.extensions.IForgeContainerType<T> {
   ...
   private static <T extends Container> ContainerType<T> register(String pKey, ContainerType.IFactory<T> pFactory) {
      return Registry.register(Registry.MENU, pKey, new ContainerType<>(pFactory));
   }

   public ContainerType(ContainerType.IFactory<T> pConstructor) {
      this.constructor = pConstructor;
   }

   @OnlyIn(Dist.CLIENT)
   public T create(int pWindowId, PlayerInventory pPlayer) {
