...
public class PlayerInteractionManager {
   ...
   public boolean destroyBlock(BlockPos pPos) {
      ...
      if (exp == -1) {
         ...
         TileEntity tileentity = this.level.getBlockEntity(pPos);
         ...
         if ((block instanceof CommandBlockBlock || block instanceof StructureBlock || block instanceof JigsawBlock) && !this.player.canUseGameMasterBlocks()) {
            ...
            if (this.isCreative()) {
               ...
               if (flag && flag1) {
                  block.playerDestroy(this.level, this.player, pPos, blockstate, tileentity, itemstack1);
               }
   ...
   public ActionResultType useItem(ServerPlayerEntity pPlayer, World pLevel, ItemStack pStack, Hand pHand) {
      if (this.gameModeForPlayer == GameType.SPECTATOR) {
         ...
         ActionResult<ItemStack> actionresult = pStack.use(pLevel, pPlayer, pHand);
         ...
         } else if (actionresult.getResult() == ActionResultType.FAIL && itemstack.getUseDuration() > 0 && !pPlayer.isUsingItem()) {
            return actionresult.getResult();
         } else {
    ...
    */
   public void setLevel(ServerWorld pServerLevel) {
