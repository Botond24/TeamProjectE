...
public abstract class PlayerList {
   ...
   public PlayerList(MinecraftServer pServer, DynamicRegistries.Impl pRegistryHolder, PlayerData pPlayerIo, int pMaxPlayers) {
      this.server = pServer;
   ...
   public ITextComponent canPlayerLogin(SocketAddress pAddress, GameProfile pProfile) {
      if (this.bans.isBanned(pProfile)) {
         ProfileBanEntry profilebanentry = this.bans.get(pProfile);
      ...
      } else if (!this.isWhiteListed(pProfile)) {
         return new TranslationTextComponent("multiplayer.disconnect.not_whitelisted");
      } else if (this.ipBans.isBanned(pAddress)) {
         IPBanEntry ipbanentry = this.ipBans.get(pAddress);
         ...
         return this.players.size() >= this.maxPlayers && !this.canBypassPlayerLimit(pProfile) ? new TranslationTextComponent("multiplayer.disconnect.server_full") : null;
      }
   }
