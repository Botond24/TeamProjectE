...
public class RenderSkyboxCube {
   ...
   public RenderSkyboxCube(ResourceLocation pTextureLocation) {
      for(int i = 0; i < 6; ++i) {
         this.images[i] = new ResourceLocation(pTextureLocation.getNamespace(), pTextureLocation.getPath() + '_' + i + ".png");
      }
   ...
   public void render(Minecraft pMc, float pPitch, float pYaw, float pAlpha) {
      ...
      RenderSystem.multMatrix(Matrix4f.perspective(85.0D, (float)pMc.getWindow().getWidth() / (float)pMc.getWindow().getHeight(), 0.05F, 10.0F));
      ...
      for(int j = 0; j < 4; ++j) {
         ...
         RenderSystem.rotatef(pPitch, 1.0F, 0.0F, 0.0F);
         RenderSystem.rotatef(pYaw, 0.0F, 1.0F, 0.0F);

         for(int k = 0; k < 6; ++k) {
            pMc.getTextureManager().bind(this.images[k]);
