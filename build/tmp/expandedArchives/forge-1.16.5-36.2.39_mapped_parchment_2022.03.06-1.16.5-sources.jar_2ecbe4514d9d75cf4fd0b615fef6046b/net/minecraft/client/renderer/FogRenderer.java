...
public class FogRenderer {
   ...
   public static void setupColor(ActiveRenderInfo pActiveRenderInfo, float pPartialTicks, ClientWorld pLevel, int pRenderDistanceChunks, float pBossColorModifier) {
      ...
      if (fluidstate.is(FluidTags.WATER)) {
         ...
         Vector3d vector3d = pLevel.getSkyColor(pActiveRenderInfo.getBlockPosition(), pPartialTicks);
         float f5 = (float)vector3d.x;
         float f8 = (float)vector3d.y;
         float f11 = (float)vector3d.z;
         float f12 = MathHelper.clamp(MathHelper.cos(pLevel.getTimeOfDay(pPartialTicks) * ((float)Math.PI * 2F)) * 2.0F + 0.5F, 0.0F, 1.0F);
         BiomeManager biomemanager = pLevel.getBiomeManager();
         Vector3d vector3d1 = pActiveRenderInfo.getPosition().subtract(2.0D, 2.0D, 2.0D).scale(0.25D);
         Vector3d vector3d2 = CubicSampler.gaussianSampleVec3(vector3d1, (p_239218_3_, p_239218_4_, p_239218_5_) -> {
            return pLevel.effects().getBrightnessDependentFogColor(Vector3d.fromRGB24(biomemanager.getNoiseBiomeAtQuart(p_239218_3_, p_239218_4_, p_239218_5_).getFogColor()), f12);
         });
         fogRed = (float)vector3d2.x();
         fogGreen = (float)vector3d2.y();
         fogBlue = (float)vector3d2.z();
         if (pRenderDistanceChunks >= 4) {
            float f13 = MathHelper.sin(pLevel.getSunAngle(pPartialTicks)) > 0.0F ? -1.0F : 1.0F;
            Vector3f vector3f = new Vector3f(f13, 0.0F, 0.0F);
            float f17 = pActiveRenderInfo.getLookVector().dot(vector3f);
            if (f17 < 0.0F) {
               f17 = 0.0F;
            }
         ...
         float f14 = pLevel.getRainLevel(pPartialTicks);
         if (f14 > 0.0F) {
         ...
         float f16 = pLevel.getThunderLevel(pPartialTicks);
         if (f16 > 0.0F) {
      ...
      double d0 = pActiveRenderInfo.getPosition().y * pLevel.getLevelData().getClearColorScale();
      if (pActiveRenderInfo.getEntity() instanceof LivingEntity && ((LivingEntity)pActiveRenderInfo.getEntity()).hasEffect(Effects.BLINDNESS)) {
         int i2 = ((LivingEntity)pActiveRenderInfo.getEntity()).getEffect(Effects.BLINDNESS).getDuration();
         if (i2 < 20) {
      ...
      if (fluidstate.is(FluidTags.WATER)) {
         float f6 = 0.0F;
         if (pActiveRenderInfo.getEntity() instanceof ClientPlayerEntity) {
            ClientPlayerEntity clientplayerentity = (ClientPlayerEntity)pActiveRenderInfo.getEntity();
      ...
      } else if (pActiveRenderInfo.getEntity() instanceof LivingEntity && ((LivingEntity)pActiveRenderInfo.getEntity()).hasEffect(Effects.NIGHT_VISION)) {
         float f7 = GameRenderer.getNightVisionScale((LivingEntity)pActiveRenderInfo.getEntity(), pPartialTicks);
   ...
   public static void setupFog(ActiveRenderInfo pActiveRenderInfo, FogRenderer.FogType pFogType, float pFarPlaneDistance, boolean pNearFog) {
      setupFog(pActiveRenderInfo, pFogType, pFarPlaneDistance, pNearFog, 0);
   }

   public static void setupFog(ActiveRenderInfo pActiveRenderInfo, FogRenderer.FogType pFogType, float pFarPlaneDistance, boolean pNearFog, float partialTicks) {
      FluidState fluidstate = pActiveRenderInfo.getFluidInCamera();
      ...
      if (fluidstate.is(FluidTags.WATER)) {
         ...
         if (fluidstate.is(FluidTags.LAVA)) {
            ...
            float f1 = MathHelper.lerp(Math.min(1.0F, (float)i / 20.0F), pFarPlaneDistance, 5.0F);
            if (pFogType == FogRenderer.FogType.FOG_SKY) {
               f2 = 0.0F;
               f3 = f1 * 0.8F;
            } else {
               f2 = f1 * 0.25F;
         ...
         } else if (pFogType == FogRenderer.FogType.FOG_SKY) {
            f2 = 0.0F;
            f3 = pFarPlaneDistance;
         } else {
            f2 = pFarPlaneDistance * 0.75F;
            f3 = pFarPlaneDistance;
         }
