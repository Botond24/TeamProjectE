...
public class WeightedBakedModel implements net.minecraftforge.client.model.data.IDynamicBakedModel {
   ...
   public List<BakedQuad> getQuads(@Nullable BlockState pState, @Nullable Direction pSide, Random pRand, net.minecraftforge.client.model.data.IModelData modelData) {
      return (WeightedRandom.getWeightedItem(this.list, Math.abs((int)pRand.nextLong()) % this.totalWeight)).model.getQuads(pState, pSide, pRand, modelData);
   }
   ...
   public static class Builder {
      ...
      public WeightedBakedModel.Builder add(@Nullable IBakedModel pModel, int pWeight) {
         if (pModel != null) {
            this.list.add(new WeightedBakedModel.WeightedModel(pModel, pWeight));
         }
