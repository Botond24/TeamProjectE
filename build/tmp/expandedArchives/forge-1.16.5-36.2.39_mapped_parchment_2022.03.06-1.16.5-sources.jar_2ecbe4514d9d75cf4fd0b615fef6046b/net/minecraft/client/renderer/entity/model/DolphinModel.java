...
public class DolphinModel<T extends Entity> extends SegmentedModel<T> {
   ...
   public void setupAnim(T pEntity, float pLimbSwing, float pLimbSwingAmount, float pAgeInTicks, float pNetHeadYaw, float pHeadPitch) {
      ...
      if (Entity.getHorizontalDistanceSqr(pEntity.getDeltaMovement()) > 1.0E-7D) {
         this.body.xRot += -0.05F + -0.05F * MathHelper.cos(pAgeInTicks * 0.3F);
         this.tail.xRot = -0.1F * MathHelper.cos(pAgeInTicks * 0.3F);
         this.tailFin.xRot = -0.2F * MathHelper.cos(pAgeInTicks * 0.3F);
      }
