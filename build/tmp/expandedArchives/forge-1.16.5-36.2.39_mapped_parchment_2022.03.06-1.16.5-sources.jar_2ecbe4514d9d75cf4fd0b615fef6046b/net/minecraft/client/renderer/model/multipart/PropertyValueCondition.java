...
public class PropertyValueCondition implements ICondition {
   ...
   private Predicate<BlockState> getBlockStatePredicate(StateContainer<Block, BlockState> pContainer, Property<?> pProperty, String pValue) {
      Optional<?> optional = pProperty.getValue(pValue);
      if (!optional.isPresent()) {
         throw new RuntimeException(String.format("Unknown value '%s' for property '%s' on '%s' in '%s'", pValue, this.key, pContainer.getOwner().toString(), this.value));
      } else {
         return (p_212483_2_) -> {
            return p_212483_2_.getValue(pProperty).equals(optional.get());
         };
      }
   }
