...
public class GuardianRenderer extends MobRenderer<GuardianEntity, GuardianModel> {
   ...
   public boolean shouldRender(GuardianEntity pLivingEntity, ClippingHelper pCamera, double pCamX, double pCamY, double pCamZ) {
      if (super.shouldRender(pLivingEntity, pCamera, pCamX, pCamY, pCamZ)) {
         ...
         if (pLivingEntity.hasActiveAttackTarget()) {
            ...
            if (livingentity != null) {
               ...
               Vector3d vector3d1 = this.getPosition(pLivingEntity, (double)pLivingEntity.getEyeHeight(), 1.0F);
               return pCamera.isVisible(new AxisAlignedBB(vector3d1.x, vector3d1.y, vector3d1.z, vector3d.x, vector3d.y, vector3d.z));
            }
         }
   ...
   public void render(GuardianEntity pEntity, float pEntityYaw, float pPartialTicks, MatrixStack pMatrixStack, IRenderTypeBuffer pBuffer, int pPackedLight) {
      ...
      if (livingentity != null) {
         ...
         Vector3d vector3d = this.getPosition(livingentity, (double)livingentity.getBbHeight() * 0.5D, pPartialTicks);
         Vector3d vector3d1 = this.getPosition(pEntity, (double)f3, pPartialTicks);
         ...
         IVertexBuilder ivertexbuilder = pBuffer.getBuffer(BEAM_RENDER_TYPE);
