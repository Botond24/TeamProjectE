...
public class SkullTileEntityRenderer extends TileEntityRenderer<SkullTileEntity> {
   ...
   public void render(SkullTileEntity pBlockEntity, float pPartialTicks, MatrixStack pMatrixStack, IRenderTypeBuffer pBuffer, int pCombinedLight, int pCombinedOverlay) {
      ...
      renderSkull(direction, f1, ((AbstractSkullBlock)blockstate.getBlock()).getType(), pBlockEntity.getOwnerProfile(), f, pMatrixStack, pBuffer, pCombinedLight);
   }
   ...
   private static RenderType getRenderType(SkullBlock.ISkullType pSkullType, @Nullable GameProfile pGameProfile) {
      ...
      if (pSkullType == SkullBlock.Types.PLAYER && pGameProfile != null) {
         ...
         return map.containsKey(Type.SKIN) ? RenderType.entityTranslucent(minecraft.getSkinManager().registerTexture(map.get(Type.SKIN), Type.SKIN)) : RenderType.entityCutoutNoCull(DefaultPlayerSkin.getDefaultSkin(PlayerEntity.createPlayerUUID(pGameProfile)));
      } else {
