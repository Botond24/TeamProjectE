...
public class DownloadingTexture extends SimpleTexture {
   ...
   public DownloadingTexture(@Nullable File pFile, String pUrlString, ResourceLocation pLocation, boolean pProcessLegacySkin, @Nullable Runnable pOnDownloaded) {
      super(pLocation);
      this.file = pFile;
   ...
   private static NativeImage processLegacySkin(NativeImage pNativeImage) {
      boolean flag = pNativeImage.getHeight() == 32;
      if (flag) {
         NativeImage nativeimage = new NativeImage(64, 64, true);
         nativeimage.copyFrom(pNativeImage);
         pNativeImage.close();
         pNativeImage = nativeimage;
      ...
      setNoAlpha(pNativeImage, 0, 0, 32, 16);
      if (flag) {
         doNotchTransparencyHack(pNativeImage, 32, 0, 64, 32);
      }

      setNoAlpha(pNativeImage, 0, 16, 64, 32);
      setNoAlpha(pNativeImage, 16, 48, 48, 64);
      return pNativeImage;
   }
