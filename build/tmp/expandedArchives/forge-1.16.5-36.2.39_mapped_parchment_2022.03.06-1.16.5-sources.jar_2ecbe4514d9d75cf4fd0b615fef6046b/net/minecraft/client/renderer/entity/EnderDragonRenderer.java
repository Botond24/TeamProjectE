...
public class EnderDragonRenderer extends EntityRenderer<EnderDragonEntity> {
   ...
   public void render(EnderDragonEntity pEntity, float pEntityYaw, float pPartialTicks, MatrixStack pMatrixStack, IRenderTypeBuffer pBuffer, int pPackedLight) {
      ...
      if (pEntity.dragonDeathTime > 0) {
         ...
         IVertexBuilder ivertexbuilder = pBuffer.getBuffer(RenderType.dragonExplosionAlpha(DRAGON_EXPLODING_LOCATION, f2));
         this.model.renderToBuffer(pMatrixStack, ivertexbuilder, pPackedLight, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 1.0F);
         IVertexBuilder ivertexbuilder1 = pBuffer.getBuffer(DECAL);
         this.model.renderToBuffer(pMatrixStack, ivertexbuilder1, pPackedLight, OverlayTexture.pack(0.0F, flag), 1.0F, 1.0F, 1.0F, 1.0F);
      } else {
         IVertexBuilder ivertexbuilder3 = pBuffer.getBuffer(RENDER_TYPE);
         this.model.renderToBuffer(pMatrixStack, ivertexbuilder3, pPackedLight, OverlayTexture.pack(0.0F, flag), 1.0F, 1.0F, 1.0F, 1.0F);
      }

      IVertexBuilder ivertexbuilder4 = pBuffer.getBuffer(EYES);
      this.model.renderToBuffer(pMatrixStack, ivertexbuilder4, pPackedLight, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 1.0F);
      if (pEntity.dragonDeathTime > 0) {
      ...
      if (pEntity.nearestCrystal != null) {
         pMatrixStack.pushPose();
         float f6 = (float)(pEntity.nearestCrystal.getX() - MathHelper.lerp((double)pPartialTicks, pEntity.xo, pEntity.getX()));
         float f8 = (float)(pEntity.nearestCrystal.getY() - MathHelper.lerp((double)pPartialTicks, pEntity.yo, pEntity.getY()));
         float f9 = (float)(pEntity.nearestCrystal.getZ() - MathHelper.lerp((double)pPartialTicks, pEntity.zo, pEntity.getZ()));
         renderCrystalBeams(f6, f8 + EnderCrystalRenderer.getY(pEntity.nearestCrystal, pPartialTicks), f9, pPartialTicks, pEntity.tickCount, pMatrixStack, pBuffer, pPackedLight);
         pMatrixStack.popPose();
      }
    ...
    */
   public ResourceLocation getTextureLocation(EnderDragonEntity pEntity) {
   ...
   public static class EnderDragonModel extends EntityModel<EnderDragonEntity> {
      ...
      public void prepareMobModel(EnderDragonEntity pEntity, float pLimbSwing, float pLimbSwingAmount, float pPartialTick) {
         this.entity = pEntity;
         this.a = pPartialTick;
      }
       ...
       */
      public void setupAnim(EnderDragonEntity pEntity, float pLimbSwing, float pLimbSwingAmount, float pAgeInTicks, float pNetHeadYaw, float pHeadPitch) {
      }
      ...
      public void renderToBuffer(MatrixStack pMatrixStack, IVertexBuilder pBuffer, int pPackedLight, int pPackedOverlay, float pRed, float pGreen, float pBlue, float pAlpha) {
         ...
         for(int i = 0; i < 5; ++i) {
            ...
            this.neck.render(pMatrixStack, pBuffer, pPackedLight, pPackedOverlay);
         }
         ...
         this.head.render(pMatrixStack, pBuffer, pPackedLight, pPackedOverlay);
         pMatrixStack.pushPose();
         pMatrixStack.translate(0.0D, 1.0D, 0.0D);
         pMatrixStack.mulPose(Vector3f.ZP.rotationDegrees(-f6 * 1.5F));
         pMatrixStack.translate(0.0D, -1.0D, 0.0D);
         this.body.zRot = 0.0F;
         this.body.render(pMatrixStack, pBuffer, pPackedLight, pPackedOverlay);
         ...
         this.renderSide(pMatrixStack, pBuffer, pPackedLight, pPackedOverlay, f1, this.leftWing, this.leftFrontLeg, this.leftFrontLegTip, this.leftFrontFoot, this.leftRearLeg, this.leftRearLegTip, this.leftRearFoot);
         this.renderSide(pMatrixStack, pBuffer, pPackedLight, pPackedOverlay, f1, this.rightWing, this.rightFrontLeg, this.rightFrontLegTip, this.rightFrontFoot, this.rightRearLeg, this.rightRearLegTip, this.rightRearFoot);
         pMatrixStack.popPose();
