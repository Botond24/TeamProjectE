...
public class BedTileEntityRenderer extends TileEntityRenderer<BedTileEntity> {
   ...
   public void render(BedTileEntity pBlockEntity, float pPartialTicks, MatrixStack pMatrixStack, IRenderTypeBuffer pBuffer, int pCombinedLight, int pCombinedOverlay) {
      RenderMaterial rendermaterial = Atlases.BED_TEXTURES[pBlockEntity.getColor().getId()];
      World world = pBlockEntity.getLevel();
      if (world != null) {
         BlockState blockstate = pBlockEntity.getBlockState();
         TileEntityMerger.ICallbackWrapper<? extends BedTileEntity> icallbackwrapper = TileEntityMerger.combineWithNeigbour(TileEntityType.BED, BedBlock::getBlockType, BedBlock::getConnectedDirection, ChestBlock.FACING, blockstate, world, pBlockEntity.getBlockPos(), (p_228846_0_, p_228846_1_) -> {
            return false;
         });
         int i = icallbackwrapper.<Int2IntFunction>apply(new DualBrightnessCallback<>()).get(pCombinedLight);
         this.renderPiece(pMatrixStack, pBuffer, blockstate.getValue(BedBlock.PART) == BedPart.HEAD, blockstate.getValue(BedBlock.FACING), rendermaterial, i, pCombinedOverlay, false);
      } else {
         this.renderPiece(pMatrixStack, pBuffer, true, Direction.SOUTH, rendermaterial, pCombinedLight, pCombinedOverlay, false);
         this.renderPiece(pMatrixStack, pBuffer, false, Direction.SOUTH, rendermaterial, pCombinedLight, pCombinedOverlay, true);
      }
