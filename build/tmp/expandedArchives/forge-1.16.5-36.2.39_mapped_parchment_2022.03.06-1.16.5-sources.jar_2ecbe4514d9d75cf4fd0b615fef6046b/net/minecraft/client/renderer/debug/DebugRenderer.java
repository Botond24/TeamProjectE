...
public class DebugRenderer {
   ...
   public void render(MatrixStack pMatrixStack, IRenderTypeBuffer.Impl pBuffer, double pCamX, double pCamY, double pCamZ) {
      if (this.renderChunkborder && !Minecraft.getInstance().showOnlyReducedInfo()) {
         this.chunkBorderRenderer.render(pMatrixStack, pBuffer, pCamX, pCamY, pCamZ);
      }

      this.gameTestDebugRenderer.render(pMatrixStack, pBuffer, pCamX, pCamY, pCamZ);
   }
   ...
   public static Optional<Entity> getTargetedEntity(@Nullable Entity pEntity, int pDistance) {
      if (pEntity == null) {
         ...
         Vector3d vector3d = pEntity.getEyePosition(1.0F);
         Vector3d vector3d1 = pEntity.getViewVector(1.0F).scale((double)pDistance);
         Vector3d vector3d2 = vector3d.add(vector3d1);
         AxisAlignedBB axisalignedbb = pEntity.getBoundingBox().expandTowards(vector3d1).inflate(1.0D);
         int i = pDistance * pDistance;
         Predicate<Entity> predicate = (p_217727_0_) -> {
            return !p_217727_0_.isSpectator() && p_217727_0_.isPickable();
         };
         EntityRayTraceResult entityraytraceresult = ProjectileHelper.getEntityHitResult(pEntity, vector3d, vector3d2, axisalignedbb, predicate, (double)i);
   ...
   public static void renderFloatingText(String pText, double p_217734_1_, double p_217734_3_, double p_217734_5_, int pColor, float p_217734_8_, boolean p_217734_9_, float p_217734_10_, boolean p_217734_11_) {
      ...
      if (activerenderinfo.isInitialized() && minecraft.getEntityRenderDispatcher().options != null) {
         ...
         float f = p_217734_9_ ? (float)(-fontrenderer.width(pText)) / 2.0F : 0.0F;
         f = f - p_217734_10_ / p_217734_8_;
         RenderSystem.enableAlphaTest();
         IRenderTypeBuffer.Impl irendertypebuffer$impl = IRenderTypeBuffer.immediate(Tessellator.getInstance().getBuilder());
         fontrenderer.drawInBatch(pText, f, 0.0F, pColor, false, TransformationMatrix.identity().getMatrix(), irendertypebuffer$impl, p_217734_11_, 0, 15728880);
   ...
   public interface IDebugRenderer {
      void render(MatrixStack pMatrixStack, IRenderTypeBuffer pBuffer, double pCamX, double pCamY, double pCamZ);
