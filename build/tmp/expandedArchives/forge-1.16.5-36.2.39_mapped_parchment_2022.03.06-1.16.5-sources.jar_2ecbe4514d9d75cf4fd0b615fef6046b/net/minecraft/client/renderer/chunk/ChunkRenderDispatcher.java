...
public class ChunkRenderDispatcher {
   ...
   public void setCamera(Vector3d pPos) {
      this.camera = pPos;
   }
   ...
   public void rebuildChunkSync(ChunkRenderDispatcher.ChunkRender pChunkRender) {
   ...
   public void schedule(ChunkRenderDispatcher.ChunkRender.ChunkRenderTask pRenderTask) {
      this.mailbox.tell(() -> {
   ...
   public class ChunkRender implements net.minecraftforge.client.extensions.IForgeRenderChunk {
      ...
      private boolean doesChunkExistAt(BlockPos pBlockPos) {
         return ChunkRenderDispatcher.this.level.getChunk(pBlockPos.getX() >> 4, pBlockPos.getZ() >> 4, ChunkStatus.FULL, false) != null;
      }
      ...
      public void setOrigin(int pX, int pY, int pZ) {
         if (pX != this.origin.getX() || pY != this.origin.getY() || pZ != this.origin.getZ()) {
            ...
            this.bb = new AxisAlignedBB((double)pX, (double)pY, (double)pZ, (double)(pX + 16), (double)(pY + 16), (double)(pZ + 16));
      ...
      private void beginLayer(BufferBuilder pBufferBuilder) {
         pBufferBuilder.begin(7, DefaultVertexFormats.BLOCK);
      }
      ...
      public void rebuildChunkAsync(ChunkRenderDispatcher pDispatcher) {
         ...
         pDispatcher.schedule(chunkrenderdispatcher$chunkrender$chunkrendertask);
      }

      private void updateGlobalBlockEntities(Set<TileEntity> pGlobalEntities) {
         Set<TileEntity> set = Sets.newHashSet(pGlobalEntities);
