...
public class EndPortalTileEntityRenderer<T extends EndPortalTileEntity> extends TileEntityRenderer<T> {
   ...
   public void render(T pBlockEntity, float pPartialTicks, MatrixStack pMatrixStack, IRenderTypeBuffer pBuffer, int pCombinedLight, int pCombinedOverlay) {
      ...
      double d0 = pBlockEntity.getBlockPos().distSqr(this.renderer.camera.getPosition(), true);
      int i = this.getPasses(d0);
      float f = this.getOffset();
      Matrix4f matrix4f = pMatrixStack.last().pose();
      this.renderCube(pBlockEntity, f, 0.15F, matrix4f, pBuffer.getBuffer(RENDER_TYPES.get(0)));

      for(int j = 1; j < i; ++j) {
         this.renderCube(pBlockEntity, f, 2.0F / (float)(18 - j), matrix4f, pBuffer.getBuffer(RENDER_TYPES.get(j)));
      }
