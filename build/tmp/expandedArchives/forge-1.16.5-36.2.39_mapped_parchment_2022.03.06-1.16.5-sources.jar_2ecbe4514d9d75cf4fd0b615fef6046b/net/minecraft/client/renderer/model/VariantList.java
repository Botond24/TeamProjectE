...
public class VariantList implements IUnbakedModel {
   ...
   public Collection<RenderMaterial> getMaterials(Function<ResourceLocation, IUnbakedModel> pModelGetter, Set<Pair<String, String>> pMissingTextureErrors) {
      return this.getVariants().stream().map(Variant::getModelLocation).distinct().flatMap((p_228831_2_) -> {
         return pModelGetter.apply(p_228831_2_).getMaterials(pModelGetter, pMissingTextureErrors).stream();
   ...
   public IBakedModel bake(ModelBakery pModelBakery, Function<RenderMaterial, TextureAtlasSprite> pSpriteGetter, IModelTransform pTransform, ResourceLocation pLocation) {
      if (this.getVariants().isEmpty()) {
         ...
         for(Variant variant : this.getVariants()) {
            IBakedModel ibakedmodel = pModelBakery.getBakedModel(variant.getModelLocation(), variant, pSpriteGetter);
