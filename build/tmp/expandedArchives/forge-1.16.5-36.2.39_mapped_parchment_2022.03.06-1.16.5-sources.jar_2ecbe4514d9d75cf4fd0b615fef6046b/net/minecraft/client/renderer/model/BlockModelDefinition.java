...
public class BlockModelDefinition {
   ...
   public static BlockModelDefinition fromStream(BlockModelDefinition.ContainerHolder pContainerHolder, Reader pReader) {
      return JSONUtils.fromJson(pContainerHolder.gson, pReader, BlockModelDefinition.class);
   }
   ...
   public static final class ContainerHolder {
      ...
      public void setDefinition(StateContainer<Block, BlockState> pStateContainer) {
   ...
   public static class Deserializer implements JsonDeserializer<BlockModelDefinition> {
      ...
      protected Map<String, VariantList> getVariants(JsonDeserializationContext pDeserializationContext, JsonObject pObject) {
         Map<String, VariantList> map = Maps.newHashMap();
         if (pObject.has("variants")) {
            JsonObject jsonobject = JSONUtils.getAsJsonObject(pObject, "variants");

            for(Entry<String, JsonElement> entry : jsonobject.entrySet()) {
               map.put(entry.getKey(), pDeserializationContext.deserialize(entry.getValue(), VariantList.class));
            }
         }
      ...
      protected Multipart getMultiPart(JsonDeserializationContext pDeserializationContext, JsonObject pObject) {
         if (!pObject.has("multipart")) {
            ...
            JsonArray jsonarray = JSONUtils.getAsJsonArray(pObject, "multipart");
