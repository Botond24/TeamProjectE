...
public class IllagerModel<T extends AbstractIllagerEntity> extends SegmentedModel<T> implements IHasArm, IHasHead {
   ...
   public void setupAnim(T pEntity, float pLimbSwing, float pLimbSwingAmount, float pAgeInTicks, float pNetHeadYaw, float pHeadPitch) {
      ...
      if (this.riding) {
         ...
         this.leftArm.yRot = 0.0F;
         this.leftArm.zRot = 0.0F;
         this.leftLeg.xRot = MathHelper.cos(pLimbSwing * 0.6662F) * 1.4F * pLimbSwingAmount * 0.5F;
         this.leftLeg.yRot = 0.0F;
         this.leftLeg.zRot = 0.0F;
         this.rightLeg.xRot = MathHelper.cos(pLimbSwing * 0.6662F + (float)Math.PI) * 1.4F * pLimbSwingAmount * 0.5F;
         this.rightLeg.yRot = 0.0F;
         this.rightLeg.zRot = 0.0F;
      }

      AbstractIllagerEntity.ArmPose abstractillagerentity$armpose = pEntity.getArmPose();
      if (abstractillagerentity$armpose == AbstractIllagerEntity.ArmPose.ATTACKING) {
         if (pEntity.getMainHandItem().isEmpty()) {
            ModelHelper.animateZombieArms(this.leftArm, this.rightArm, true, this.attackTime, pAgeInTicks);
         } else {
            ModelHelper.swingWeaponDown(this.rightArm, this.leftArm, pEntity, this.attackTime, pAgeInTicks);
         }
         ...
         this.leftArm.z = 0.0F;
         this.leftArm.x = 5.0F;
         this.rightArm.xRot = MathHelper.cos(pAgeInTicks * 0.6662F) * 0.25F;
         this.leftArm.xRot = MathHelper.cos(pAgeInTicks * 0.6662F) * 0.25F;
         ...
         ModelHelper.animateCrossbowCharge(this.rightArm, this.leftArm, pEntity, true);
      } else if (abstractillagerentity$armpose == AbstractIllagerEntity.ArmPose.CELEBRATING) {
         this.rightArm.z = 0.0F;
         this.rightArm.x = -5.0F;
         this.rightArm.xRot = MathHelper.cos(pAgeInTicks * 0.6662F) * 0.05F;
         this.rightArm.zRot = 2.670354F;
         this.rightArm.yRot = 0.0F;
         this.leftArm.z = 0.0F;
         this.leftArm.x = 5.0F;
         this.leftArm.xRot = MathHelper.cos(pAgeInTicks * 0.6662F) * 0.05F;
         this.leftArm.zRot = -2.3561945F;
         this.leftArm.yRot = 0.0F;
      }
