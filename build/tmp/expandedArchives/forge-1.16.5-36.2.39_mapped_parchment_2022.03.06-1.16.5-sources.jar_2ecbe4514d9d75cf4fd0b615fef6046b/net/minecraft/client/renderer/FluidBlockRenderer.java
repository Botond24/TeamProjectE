...
public class FluidBlockRenderer {
   ...
   public boolean tesselate(IBlockDisplayReader pLightReader, BlockPos pPos, IVertexBuilder pVertexBuilder, FluidState pFluidState) {
      ...
      if (!flag1 && !flag2 && !flag6 && !flag5 && !flag3 && !flag4) {
         ...
         if (flag1 && !isFaceOccludedByNeighbor(pLightReader, pPos, Direction.UP, Math.min(Math.min(f7, f8), Math.min(f9, f10)))) {
            flag7 = true;
            f7 -= 0.001F;
            f8 -= 0.001F;
            f9 -= 0.001F;
   ...
   @Deprecated
   private void vertex(IVertexBuilder pVertexBuilder, double pX, double pY, double pZ, float pRed, float pGreen, float pBlue, float pU, float pV, int pPackedLight) {
       vertexVanilla(pVertexBuilder, pX, pY, pZ, pRed, pGreen, pBlue, 1.0F, pU, pV, pPackedLight);
   }
