...
public class BlazeModel<T extends Entity> extends SegmentedModel<T> {
   ...
   public void setupAnim(T pEntity, float pLimbSwing, float pLimbSwingAmount, float pAgeInTicks, float pNetHeadYaw, float pHeadPitch) {
      ...
      for(int i = 0; i < 4; ++i) {
         this.upperBodyParts[i].y = -2.0F + MathHelper.cos(((float)(i * 2) + pAgeInTicks) * 0.25F);
      ...
      for(int j = 4; j < 8; ++j) {
         this.upperBodyParts[j].y = 2.0F + MathHelper.cos(((float)(j * 2) + pAgeInTicks) * 0.25F);
      ...
      for(int k = 8; k < 12; ++k) {
         this.upperBodyParts[k].y = 11.0F + MathHelper.cos(((float)k * 1.5F + pAgeInTicks) * 0.5F);
