...
public class RavagerModel extends SegmentedModel<RavagerEntity> {
    ...
    */
   public void setupAnim(RavagerEntity pEntity, float pLimbSwing, float pLimbSwingAmount, float pAgeInTicks, float pNetHeadYaw, float pHeadPitch) {
      this.head.xRot = pHeadPitch * ((float)Math.PI / 180F);
      this.head.yRot = pNetHeadYaw * ((float)Math.PI / 180F);
      this.body.xRot = ((float)Math.PI / 2F);
      float f = 0.4F * pLimbSwingAmount;
      this.leg0.xRot = MathHelper.cos(pLimbSwing * 0.6662F) * f;
      this.leg1.xRot = MathHelper.cos(pLimbSwing * 0.6662F + (float)Math.PI) * f;
      this.leg2.xRot = MathHelper.cos(pLimbSwing * 0.6662F + (float)Math.PI) * f;
      this.leg3.xRot = MathHelper.cos(pLimbSwing * 0.6662F) * f;
   }

   public void prepareMobModel(RavagerEntity pEntity, float pLimbSwing, float pLimbSwingAmount, float pPartialTick) {
      ...
      int i1 = 10;
      if (l > 0) {
         float f = MathHelper.triangleWave((float)l - pPartialTick, 10.0F);
         float f1 = (1.0F + f) * 0.5F;
         float f2 = f1 * f1 * f1 * 12.0F;
         float f3 = f2 * MathHelper.sin(this.neck.xRot);
         this.neck.z = -6.5F + f2;
         this.neck.y = -7.0F - f3;
         float f4 = MathHelper.sin(((float)l - pPartialTick) / 10.0F * (float)Math.PI * 0.25F);
         this.mouth.xRot = ((float)Math.PI / 2F) * f4;
         if (l > 5) {
            this.mouth.xRot = MathHelper.sin(((float)(-4 + l) - pPartialTick) / 4.0F) * (float)Math.PI * 0.4F;
         } else {
            this.mouth.xRot = 0.15707964F * MathHelper.sin((float)Math.PI * ((float)l - pPartialTick) / 10.0F);
         }
      } else {
         float f5 = -1.0F;
