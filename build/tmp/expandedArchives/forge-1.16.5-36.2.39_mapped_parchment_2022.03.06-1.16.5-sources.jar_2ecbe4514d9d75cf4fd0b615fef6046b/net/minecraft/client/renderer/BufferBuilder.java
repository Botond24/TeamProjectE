...
public class BufferBuilder extends DefaultColorVertexBuilder implements IVertexConsumer {
   ...
   public BufferBuilder(int pCapacity) {
      this.buffer = GLAllocation.createByteBuffer(pCapacity * 4);
   }
   ...
   private void switchFormat(VertexFormat pFormat) {
      if (this.format != pFormat) {
         this.format = pFormat;
         boolean flag = pFormat == DefaultVertexFormats.NEW_ENTITY;
         boolean flag1 = pFormat == DefaultVertexFormats.BLOCK;
   ...
   public IVertexBuilder color(int pRed, int pGreen, int pBlue, int pAlpha) {
      if (this.defaultColorSet) {
         ...
         return IVertexConsumer.super.color(pRed, pGreen, pBlue, pAlpha);
      }
   }
   ...
   public void vertex(float pX, float pY, float pZ, float pRed, float pGreen, float pBlue, float pAlpha, float pTexU, float pTexV, int pOverlayUV, int pLightmapUV, float pNormalX, float pNormalY, float pNormalZ) {
      if (this.defaultColorSet) {
         ...
         this.putByte(i + 4, IVertexConsumer.normalIntValue(pNormalX));
         this.putByte(i + 5, IVertexConsumer.normalIntValue(pNormalY));
         this.putByte(i + 6, IVertexConsumer.normalIntValue(pNormalZ));
