...
public class ArmorStandRenderer extends LivingRenderer<ArmorStandEntity, ArmorStandArmorModel> {
    ...
    */
   public ResourceLocation getTextureLocation(ArmorStandEntity pEntity) {
      return DEFAULT_SKIN_LOCATION;
   }

   protected void setupRotations(ArmorStandEntity pEntityLiving, MatrixStack pMatrixStack, float pAgeInTicks, float pRotationYaw, float pPartialTicks) {
      ...
      if (f < 5.0F) {
         pMatrixStack.mulPose(Vector3f.YP.rotationDegrees(MathHelper.sin(f / 1.5F * (float)Math.PI) * 3.0F));
      }

   }

   protected boolean shouldShowName(ArmorStandEntity pEntity) {
