...
public class Selector {
   ...
   public Predicate<BlockState> getPredicate(StateContainer<Block, BlockState> pState) {
   ...
   public static class Deserializer implements JsonDeserializer<Selector> {
      ...
      private ICondition getSelector(JsonObject pJson) {
         return pJson.has("when") ? getCondition(JSONUtils.getAsJsonObject(pJson, "when")) : ICondition.TRUE;
      }

      @VisibleForTesting
      static ICondition getCondition(JsonObject pJson) {
         Set<Entry<String, JsonElement>> set = pJson.entrySet();
         if (set.isEmpty()) {
            throw new JsonParseException("No elements found in selector");
         } else if (set.size() == 1) {
            if (pJson.has("OR")) {
               List<ICondition> list1 = Streams.stream(JSONUtils.getAsJsonArray(pJson, "OR")).map((p_200692_0_) -> {
                  return getCondition(p_200692_0_.getAsJsonObject());
               }).collect(Collectors.toList());
               return new OrCondition(list1);
            } else if (pJson.has("AND")) {
               List<ICondition> list = Streams.stream(JSONUtils.getAsJsonArray(pJson, "AND")).map((p_200691_0_) -> {
