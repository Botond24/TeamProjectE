...
public class StructureTileEntityRenderer extends TileEntityRenderer<StructureBlockTileEntity> {
   ...
   public void render(StructureBlockTileEntity pBlockEntity, float pPartialTicks, MatrixStack pMatrixStack, IRenderTypeBuffer pBuffer, int pCombinedLight, int pCombinedOverlay) {
      if (Minecraft.getInstance().player.canUseGameMasterBlocks() || Minecraft.getInstance().player.isSpectator()) {
         BlockPos blockpos = pBlockEntity.getStructurePos();
         BlockPos blockpos1 = pBlockEntity.getStructureSize();
         if (blockpos1.getX() >= 1 && blockpos1.getY() >= 1 && blockpos1.getZ() >= 1) {
            if (pBlockEntity.getMode() == StructureMode.SAVE || pBlockEntity.getMode() == StructureMode.LOAD) {
               ...
               if (pBlockEntity.getMode() == StructureMode.SAVE || pBlockEntity.getShowBoundingBox()) {
                  WorldRenderer.renderLineBox(pMatrixStack, ivertexbuilder, d4, d5, d6, d7, d8, d9, 0.9F, 0.9F, 0.9F, 1.0F, 0.5F, 0.5F, 0.5F);
               }

               if (pBlockEntity.getMode() == StructureMode.SAVE && pBlockEntity.getShowAir()) {
                  this.renderInvisibleBlocks(pBlockEntity, ivertexbuilder, blockpos, true, pMatrixStack);
                  this.renderInvisibleBlocks(pBlockEntity, ivertexbuilder, blockpos, false, pMatrixStack);
               }
   ...
   public boolean shouldRenderOffScreen(StructureBlockTileEntity pTe) {
      return true;
   }
}