...
public class TextureAtlasSprite implements AutoCloseable, net.minecraftforge.client.extensions.IForgeTextureAtlasSprite {
   ...
   protected TextureAtlasSprite(AtlasTexture pAtlas, TextureAtlasSprite.Info p_i226049_2_, int p_i226049_3_, int p_i226049_4_, int p_i226049_5_, int pX, int pY, NativeImage p_i226049_8_) {
      this.atlas = pAtlas;
      ...
      this.x = pX;
      this.y = pY;
      this.u0 = (float)pX / (float)p_i226049_4_;
      this.u1 = (float)(pX + i) / (float)p_i226049_4_;
      this.v0 = (float)pY / (float)p_i226049_5_;
      this.v1 = (float)(pY + j) / (float)p_i226049_5_;
   ...
   private void upload(int pXOffset, int pYOffset, NativeImage[] pFrames) {
      for(int i = 0; i < this.mainImage.length; ++i) {
         if ((this.info.width >> i <= 0) || (this.info.height >> i <= 0)) break;
         pFrames[i].upload(i, this.x >> i, this.y >> i, pXOffset >> i, pYOffset >> i, this.info.width >> i, this.info.height >> i, this.mainImage.length > 1, false);
      }
   ...
   public boolean isTransparent(int pFrameIndex, int pPixelX, int pPixelY) {
      return (this.mainImage[0].getPixelRGBA(pPixelX + this.framesX[pFrameIndex] * this.info.width, pPixelY + this.framesY[pFrameIndex] * this.info.height) >> 24 & 255) == 0;
   }
   ...
   public IVertexBuilder wrap(IVertexBuilder pBuffer) {
      return new SpriteAwareVertexBuilder(pBuffer, this);
   }
   ...
   final class InterpolationData implements AutoCloseable {
      ...
      private int mix(double pRatio, int pVal1, int pVal2) {
         return (int)(pRatio * (double)pVal1 + (1.0D - pRatio) * (double)pVal2);
      }
