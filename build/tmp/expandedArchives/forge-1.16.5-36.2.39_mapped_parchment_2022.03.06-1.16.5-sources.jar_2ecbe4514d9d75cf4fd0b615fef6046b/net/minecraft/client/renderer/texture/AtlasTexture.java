...
public class AtlasTexture extends Texture implements ITickable {
   ...
   public AtlasTexture(ResourceLocation pLocation) {
   ...
   public void reload(AtlasTexture.SheetData pSheetData) {
      this.sprites.clear();
   ...
   public AtlasTexture.SheetData prepareToStitch(IResourceManager pResourceManager, Stream<ResourceLocation> pResourceLocations, IProfiler pProfiler, int pMaxMipmapLevel) {
      pProfiler.push("preparing");
      Set<ResourceLocation> set = pResourceLocations.peek((p_229222_0_) -> {
         if (p_229222_0_ == null) {
            throw new IllegalArgumentException("Location cannot be null!");
         }
      }).collect(Collectors.toSet());
      int i = this.maxSupportedTextureSize;
      Stitcher stitcher = new Stitcher(i, i, pMaxMipmapLevel);
      int j = Integer.MAX_VALUE;
      int k = 1 << pMaxMipmapLevel;
      ...
      int k1 = pMaxMipmapLevel;
      if (false) // FORGE: do not lower the mipmap level
      if (j1 < pMaxMipmapLevel) {
         LOGGER.warn("{}: dropping miplevel from {} to {}, because of minimum power of two: {}", this.location, pMaxMipmapLevel, j1, i1);
         k1 = j1;
      } else {
         k1 = pMaxMipmapLevel;
      }
   ...
   private Collection<TextureAtlasSprite.Info> getBasicSpriteInfos(IResourceManager pResourceManager, Set<ResourceLocation> pSpriteLocations) {
      ...
      for(ResourceLocation resourcelocation : pSpriteLocations) {
         if (!MissingTextureSprite.getLocation().equals(resourcelocation)) {
            list.add(CompletableFuture.runAsync(() -> {
               ...
               try (IResource iresource = pResourceManager.getResource(resourcelocation1)) {
   ...
   private List<TextureAtlasSprite> getLoadedSprites(IResourceManager pResourceManager, Stitcher pStitcher, int pMipmapLevel) {
      ConcurrentLinkedQueue<TextureAtlasSprite> concurrentlinkedqueue = new ConcurrentLinkedQueue<>();
      List<CompletableFuture<?>> list = Lists.newArrayList();
      pStitcher.gatherSprites((p_229215_5_, p_229215_6_, p_229215_7_, p_229215_8_, p_229215_9_) -> {
         if (p_229215_5_ == MissingTextureSprite.info()) {
            MissingTextureSprite missingtexturesprite = MissingTextureSprite.newInstance(this, pMipmapLevel, p_229215_6_, p_229215_7_, p_229215_8_, p_229215_9_);
            concurrentlinkedqueue.add(missingtexturesprite);
         } else {
            list.add(CompletableFuture.runAsync(() -> {
               TextureAtlasSprite textureatlassprite = this.load(pResourceManager, p_229215_5_, p_229215_6_, p_229215_7_, pMipmapLevel, p_229215_8_, p_229215_9_);
   ...
   @Nullable
   private TextureAtlasSprite load(IResourceManager pResourceManager, TextureAtlasSprite.Info pSpriteInfo, int pWidth, int pHeight, int pMipmapLevel, int pOriginX, int pOriginY) {
      ...
      try (IResource iresource = pResourceManager.getResource(resourcelocation)) {
         ...
         TextureAtlasSprite customSprite = net.minecraftforge.client.ForgeHooksClient.loadTextureAtlasSprite(this, pResourceManager, pSpriteInfo, iresource, pWidth, pHeight, pOriginX, pOriginY, pMipmapLevel, nativeimage);
         if (customSprite != null) return customSprite;
         return new TextureAtlasSprite(this, pSpriteInfo, pMipmapLevel, pWidth, pHeight, pOriginX, pOriginY, nativeimage);
      } catch (RuntimeException runtimeexception) {
   ...
   private ResourceLocation getResourceLocation(ResourceLocation pLocation) {
      return new ResourceLocation(pLocation.getNamespace(), String.format("textures/%s%s", pLocation.getPath(), ".png"));
   }
   ...
   public void updateFilter(AtlasTexture.SheetData pSheetData) {
   ...
   public static class SheetData {
      ...
      public SheetData(Set<ResourceLocation> pSprites, int pWidth, int pHeight, int pMipLevel, List<TextureAtlasSprite> pRegions) {
