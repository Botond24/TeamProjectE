...
public class FindTreeStep implements ITutorialStep {
   ...
   public FindTreeStep(Tutorial pTutorial) {
    ...
    */
   public void onLookAt(ClientWorld pLevel, RayTraceResult pResult) {
      if (pResult.getType() == RayTraceResult.Type.BLOCK) {
         BlockState blockstate = pLevel.getBlockState(((BlockRayTraceResult)pResult).getBlockPos());
   ...
   public void onGetItem(ItemStack pStack) {
      for(Block block : TREE_BLOCKS) {
         if (pStack.getItem() == block.asItem()) {
            this.tutorial.setStep(TutorialSteps.CRAFT_PLANKS);
            return;
         }
      }
   ...
   public static boolean hasPunchedTreesPreviously(ClientPlayerEntity pPlayer) {
      for(Block block : TREE_BLOCKS) {
