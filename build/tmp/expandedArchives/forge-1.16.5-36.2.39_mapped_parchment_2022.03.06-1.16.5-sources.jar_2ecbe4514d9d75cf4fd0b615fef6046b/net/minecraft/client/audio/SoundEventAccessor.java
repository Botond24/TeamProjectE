...
public class SoundEventAccessor implements ISoundEventAccessor<Sound> {
   ...
   public SoundEventAccessor(ResourceLocation pLocation, @Nullable String pSubtitle) {
      this.location = pLocation;
      this.subtitle = pSubtitle == null ? null : new TranslationTextComponent(pSubtitle);
   }
   ...
   public void addSound(ISoundEventAccessor<Sound> pAccessor) {
   ...
   public void preloadIfRequired(SoundEngine pEngine) {
      for(ISoundEventAccessor<Sound> isoundeventaccessor : this.list) {
         isoundeventaccessor.preloadIfRequired(pEngine);
      }
