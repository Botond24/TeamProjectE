...
public class Minecraft extends RecursiveEventLoop<Runnable> implements ISnooperInfo, IWindowEventListener, net.minecraftforge.client.extensions.IForgeMinecraft {
   ...
   public Minecraft(GameConfiguration pGameConfig) {
      super("Client");
      instance = this;
      ...
      this.clientPackSource = new DownloadingPackFinder(new File(this.gameDirectory, "server-resource-packs"), pGameConfig.location.getAssetIndex());
      ...
      if (this.options.overrideHeight > 0 && this.options.overrideWidth > 0) {
         screensize = new ScreenSize(this.options.overrideWidth, this.options.overrideHeight, pGameConfig.display.fullscreenWidth, pGameConfig.display.fullscreenHeight, pGameConfig.display.isFullscreen);
      } else {
         screensize = pGameConfig.display;
      }
   ...
   private SocialInteractionsService createSocialInteractions(YggdrasilAuthenticationService pAuthService, GameConfiguration pGameConfig) {
      try {
   ...
   public void clearResourcePacksOnError(Throwable pThrowable, @Nullable ITextComponent pErrorMessage) {
      ...
      this.reloadResourcePacks().thenRun(() -> {
         ...
         SystemToast.addOrUpdate(toastgui, SystemToast.Type.PACK_LOAD_FAILURE, new TranslationTextComponent("resourcePack.load_fail"), pErrorMessage);
      });
   }
   ...
   public void setScreen(@Nullable Screen pGuiScreen) {
      if (pGuiScreen == null && this.level == null) {
         pGuiScreen = new MainMenuScreen();
      } else if (pGuiScreen == null && this.player.isDeadOrDying()) {
         if (this.player.shouldShowDeathScreen()) {
            pGuiScreen = new DeathScreen((ITextComponent)null, this.level.getLevelData().isHardcore());
         } else {
            this.player.respawn();
         }
      }
      ...
      net.minecraftforge.client.event.GuiOpenEvent event = new net.minecraftforge.client.event.GuiOpenEvent(pGuiScreen);
      if (net.minecraftforge.common.MinecraftForge.EVENT_BUS.post(event)) return;

      pGuiScreen = event.getGui();
      if (old != null && pGuiScreen != old)
         old.removed();

      if (pGuiScreen instanceof MainMenuScreen || pGuiScreen instanceof MultiplayerScreen) {
   ...
   public void setOverlay(@Nullable LoadingGui pLoadingGui) {
    ...
    */
   void debugFpsMeterKeyPress(int pKeyCount) {
      if (this.fpsPieResults != null) {
   ...
   private void renderFpsMeter(MatrixStack pMatrixStack, IProfileResult pProfilerResult) {
      List<DataPoint> list = pProfilerResult.getTimes(this.debugPath);
      DataPoint datapoint = list.remove(0);
      ...
      for(int j2 = 0; j2 < list.size(); ++j2) {
         ...
         this.font.drawShadow(pMatrixStack, s2, (float)(j - 160), (float)(k + 80 + j2 * 8 + 20), datapoint2.getColor());
         s2 = decimalformat.format(datapoint2.percentage) + "%";
         this.font.drawShadow(pMatrixStack, s2, (float)(j + 160 - 50 - this.font.width(s2)), (float)(k + 80 + j2 * 8 + 20), datapoint2.getColor());
         s2 = decimalformat.format(datapoint2.globalPercentage) + "%";
         this.font.drawShadow(pMatrixStack, s2, (float)(j + 160 - this.font.width(s2)), (float)(k + 80 + j2 * 8 + 20), datapoint2.getColor());
      }
   ...
   public void pauseGame(boolean pPauseOnly) {
      if (this.screen == null) {
         ...
         if (flag) {
            this.setScreen(new IngameMenuScreen(!pPauseOnly));
   ...
   private void continueAttack(boolean pLeftClick) {
      ...
      if (this.missTime <= 0 && !this.player.isUsingItem()) {
         if (pLeftClick && this.hitResult != null && this.hitResult.getType() == RayTraceResult.Type.BLOCK) {
   ...
   public void loadLevel(String pLevelName) {
      this.loadWorld(pLevelName, DynamicRegistries.builtin(), Minecraft::loadDataPacks, Minecraft::loadWorldData, false, Minecraft.WorldSelectionType.BACKUP, false);
   }

   public void createLevel(String pLevelName, WorldSettings pLevelSettings, DynamicRegistries.Impl pDynamicRegistries, DimensionGeneratorSettings pDimensionGeneratorSettings) {
      this.loadWorld(pLevelName, pDynamicRegistries, (p_238179_1_) -> {
         return pLevelSettings.getDataPackConfig();
      }, (p_238187_3_, p_238187_4_, p_238187_5_, p_238187_6_) -> {
         WorldGenSettingsExport<JsonElement> worldgensettingsexport = WorldGenSettingsExport.create(JsonOps.INSTANCE, pDynamicRegistries);
         WorldSettingsImport<JsonElement> worldsettingsimport = WorldSettingsImport.create(JsonOps.INSTANCE, p_238187_5_, pDynamicRegistries);
         DataResult<DimensionGeneratorSettings> dataresult = DimensionGeneratorSettings.CODEC.encodeStart(worldgensettingsexport, pDimensionGeneratorSettings).setLifecycle(Lifecycle.stable()).flatMap((p_243209_1_) -> {
            return DimensionGeneratorSettings.CODEC.parse(worldsettingsimport, p_243209_1_);
         });
         DimensionGeneratorSettings dimensiongeneratorsettings = dataresult.resultOrPartial(Util.prefix("Error reading worldgen settings after loading data packs: ", LOGGER::error)).orElse(pDimensionGeneratorSettings);
         return new ServerWorldInfo(pLevelSettings, dimensiongeneratorsettings, dataresult.lifecycle());
      }, false, Minecraft.WorldSelectionType.CREATE, true);
   }

   private void loadWorld(String pLevelName, DynamicRegistries.Impl pDynamicRegistries, Function<SaveFormat.LevelSave, DatapackCodec> pLevelSaveToDatapackFunction, Function4<SaveFormat.LevelSave, DynamicRegistries.Impl, IResourceManager, DatapackCodec, IServerConfiguration> pQuadFunction, boolean pVanillaOnly, Minecraft.WorldSelectionType pSelectionType, boolean creating) {
      SaveFormat.LevelSave saveformat$levelsave;
      try {
         saveformat$levelsave = this.levelSource.createAccess(pLevelName);
      ...
      try {
         Minecraft.PackManager mgr = this.makeServerStem(pDynamicRegistries, pLevelSaveToDatapackFunction, pQuadFunction, pVanillaOnly, saveformat$levelsave);
         ...
         dyn_f = creating ? pDynamicRegistries : DynamicRegistries.builtin();
         minecraft$packmanager = creating ? mgr : this.makeServerStem(dyn_f, pLevelSaveToDatapackFunction, pQuadFunction, pVanillaOnly, saveformat$levelsave); //Note this runs the injection again... which isn't good.. but hey.. we need a better spot to hook in.

      } catch (Exception exception) {
         LOGGER.warn("Failed to load datapacks, can't proceed with server load", (Throwable)exception);
         this.setScreen(new DatapackFailureScreen(() -> {
            this.loadWorld(pLevelName, pDynamicRegistries, pLevelSaveToDatapackFunction, pQuadFunction, true, pSelectionType, creating);
         }));
      ...
      if (pSelectionType == Minecraft.WorldSelectionType.NONE || !flag && !flag1) {
         this.clearLevel();
         ...
         this.displayExperimentalConfirmationDialog(pSelectionType, pLevelName, flag, () -> {
            this.loadWorld(pLevelName, dyn_f, pLevelSaveToDatapackFunction, pQuadFunction, pVanillaOnly, Minecraft.WorldSelectionType.NONE, creating);
         });
   ...
   private void displayExperimentalConfirmationDialog(Minecraft.WorldSelectionType pSelectionType, String pLevelName, boolean pCustomized, Runnable pRunnable) {
      if (pSelectionType == Minecraft.WorldSelectionType.BACKUP) {
         ITextComponent itextcomponent;
         ...
         this.setScreen(new ConfirmScreen((p_241560_3_) -> {
            if (p_241560_3_) {
               ...
               try (SaveFormat.LevelSave saveformat$levelsave = this.levelSource.createAccess(pLevelName)) {
   ...
   public Minecraft.PackManager makeServerStem(DynamicRegistries.Impl pDynamicRegistries, Function<SaveFormat.LevelSave, DatapackCodec> pLevelStorageToDatapackFunction, Function4<SaveFormat.LevelSave, DynamicRegistries.Impl, IResourceManager, DatapackCodec, IServerConfiguration> pQuadFunction, boolean pVanillaOnly, SaveFormat.LevelSave pLevelStorage) throws InterruptedException, ExecutionException {
      DatapackCodec datapackcodec = pLevelStorageToDatapackFunction.apply(pLevelStorage);
      ResourcePackList resourcepacklist = new ResourcePackList(new ServerPackFinder(), new FolderPackFinder(pLevelStorage.getLevelPath(FolderName.DATAPACK_DIR).toFile(), IPackNameDecorator.WORLD));

      try {
         DatapackCodec datapackcodec1 = MinecraftServer.configurePackRepository(resourcepacklist, datapackcodec, pVanillaOnly);
         ...
         IServerConfiguration iserverconfiguration = pQuadFunction.apply(pLevelStorage, pDynamicRegistries, datapackregistries.getResourceManager(), datapackcodec1);
    ...
    */
   public void setLevel(ClientWorld pLevelClient) {
   ...
   private void updateLevelInEngines(@Nullable ClientWorld pLevel) {
      this.levelRenderer.setLevel(pLevel);
      this.particleEngine.setLevel(pLevel);
      TileEntityRendererDispatcher.instance.setLevel(pLevel);
      this.updateTitle();
   ...
   public ItemStack addCustomNbtData(ItemStack pStack, TileEntity pTe) {
      CompoundNBT compoundnbt = pTe.save(new CompoundNBT());
      if (pStack.getItem() instanceof SkullItem && compoundnbt.contains("SkullOwner")) {
         CompoundNBT compoundnbt2 = compoundnbt.getCompound("SkullOwner");
         pStack.getOrCreateTag().put("SkullOwner", compoundnbt2);
         return pStack;
      } else {
         pStack.addTagElement("BlockEntityTag", compoundnbt);
         CompoundNBT compoundnbt1 = new CompoundNBT();
         ListNBT listnbt = new ListNBT();
         listnbt.add(StringNBT.valueOf("\"(+NBT)\""));
         compoundnbt1.put("Lore", listnbt);
         pStack.addTagElement("display", compoundnbt1);
         return pStack;
      }
   }
   ...
   public CrashReport fillReport(CrashReport pTheCrash) {
      fillReport(this.languageManager, this.launchedVersion, this.options, pTheCrash);
      if (this.level != null) {
   ...
   public void populateSnooper(Snooper pSnooper) {
      ...
      for(ResourcePackInfo resourcepackinfo : this.resourcePackRepository.getSelectedPacks()) {
         if (!resourcepackinfo.isRequired() && !resourcepackinfo.isFixedPosition()) {
            pSnooper.setDynamicData("resource_pack[" + i++ + "]", resourcepackinfo.getId());
         }
      }
   ...
   public boolean shouldEntityAppearGlowing(Entity pEntity) {
      return pEntity.isGlowing() || this.player != null && this.player.isSpectator() && this.options.keySpectatorOutlines.isDown() && pEntity.getType() == EntityType.PLAYER;
   }
   ...
   public <T> IMutableSearchTree<T> getSearchTree(SearchTreeManager.Key<T> pKey) {
   ...
   private static Supplier<IResourcePack> adaptV3(Supplier<IResourcePack> pResourcePackSupplier) {
      return () -> {
         return new LegacyResourcePackWrapper(pResourcePackSupplier.get(), LegacyResourcePackWrapper.V3);
      };
   }

   private static Supplier<IResourcePack> adaptV4(Supplier<IResourcePack> pResourcePackSupplier) {
      return () -> {
         return new LegacyResourcePackWrapperV4(pResourcePackSupplier.get());
      };
   }
   ...
   public static final class PackManager implements AutoCloseable {
      ...
      private PackManager(ResourcePackList pPackRepository, DataPackRegistries pServerResources, IServerConfiguration pWorldData) {
