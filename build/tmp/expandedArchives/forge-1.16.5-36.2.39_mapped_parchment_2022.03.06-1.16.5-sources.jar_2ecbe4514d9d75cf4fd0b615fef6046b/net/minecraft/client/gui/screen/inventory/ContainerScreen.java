...
public abstract class ContainerScreen<T extends Container> extends Screen implements IHasContainer<T> {
   ...
   public ContainerScreen(T pMenu, PlayerInventory pPlayerInventory, ITextComponent pTitle) {
      super(pTitle);
      this.menu = pMenu;
   ...
   private void renderSlot(MatrixStack pPoseStack, Slot pSlot) {
      ...
      if (pSlot == this.clickedSlot && !this.draggingItem.isEmpty() && this.isSplittingStack && !itemstack.isEmpty()) {
         ...
         if (Container.canItemQuickReplace(pSlot, itemstack1, true) && this.menu.canDragTo(pSlot)) {
            ...
            flag = true;
            Container.getQuickCraftSlotCount(this.quickCraftSlots, this.quickCraftingType, itemstack, pSlot.getItem().isEmpty() ? 0 : pSlot.getItem().getCount());
   ...
   protected boolean checkHotbarKeyPressed(int pKeyCode, int pScanCode) {
      if (this.minecraft.player.inventory.getCarried().isEmpty() && this.hoveredSlot != null) {
         ...
         for(int i = 0; i < 9; ++i) {
            if (this.minecraft.options.keyHotbarSlots[i].isActiveAndMatches(InputMappings.getKey(pKeyCode, pScanCode))) {
               this.slotClicked(this.hoveredSlot, this.hoveredSlot.index, i, ClickType.SWAP);
               return true;
            }
         }
