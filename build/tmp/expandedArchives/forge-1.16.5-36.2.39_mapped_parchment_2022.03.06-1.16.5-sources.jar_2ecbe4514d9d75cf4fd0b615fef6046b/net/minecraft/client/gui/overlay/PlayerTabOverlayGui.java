...
public class PlayerTabOverlayGui extends AbstractGui {
   ...
   public PlayerTabOverlayGui(Minecraft pMinecraft, IngameGui pGui) {
      this.minecraft = pMinecraft;
      this.gui = pGui;
   }

   public ITextComponent getNameForDisplay(NetworkPlayerInfo pPlayerInfo) {
      return pPlayerInfo.getTabListDisplayName() != null ? this.decorateName(pPlayerInfo, pPlayerInfo.getTabListDisplayName().copy()) : this.decorateName(pPlayerInfo, ScorePlayerTeam.formatNameForTeam(pPlayerInfo.getTeam(), new StringTextComponent(pPlayerInfo.getProfile().getName())));
   }

   private ITextComponent decorateName(NetworkPlayerInfo pPlayerInfo, IFormattableTextComponent pName) {
      return pPlayerInfo.getGameMode() == GameType.SPECTATOR ? pName.withStyle(TextFormatting.ITALIC) : pName;
   }
   ...
   public void render(MatrixStack pPoseStack, int p_238523_2_, Scoreboard pScoreboard, @Nullable ScoreObjective pObjective) {
      ...
      for(NetworkPlayerInfo networkplayerinfo : list) {
         ...
         i = Math.max(i, k);
         if (pObjective != null && pObjective.getRenderType() != ScoreCriteria.RenderType.HEARTS) {
            k = this.minecraft.font.width(" " + pScoreboard.getOrCreatePlayerScore(networkplayerinfo.getProfile().getName(), pObjective).getScore());
            j = Math.max(j, k);
         }
      }
      ...
      int l;
      if (pObjective != null) {
         if (pObjective.getRenderType() == ScoreCriteria.RenderType.HEARTS) {
            l = 90;
         } else {
            l = j;
         }
      ...
      if (list1 != null) {
         fill(pPoseStack, p_238523_2_ / 2 - l1 / 2 - 1, k1 - 1, p_238523_2_ / 2 + l1 / 2 + 1, k1 + list1.size() * 9, Integer.MIN_VALUE);

         for(IReorderingProcessor ireorderingprocessor2 : list1) {
            int i2 = this.minecraft.font.width(ireorderingprocessor2);
            this.minecraft.font.drawShadow(pPoseStack, ireorderingprocessor2, (float)(p_238523_2_ / 2 - i2 / 2), (float)k1, -1);
            k1 += 9;
         }
      ...
      fill(pPoseStack, p_238523_2_ / 2 - l1 / 2 - 1, k1 - 1, p_238523_2_ / 2 + l1 / 2 + 1, k1 + j4 * 9, Integer.MIN_VALUE);
      ...
      for(int i5 = 0; i5 < i4; ++i5) {
         ...
         int l2 = k1 + j2 * 9;
         fill(pPoseStack, k2, l2, k2 + i1, l2 + 8, l4);
         ...
         if (i5 < list.size()) {
            ...
            if (flag) {
               ...
               int i3 = 8 + (flag1 ? 8 : 0);
               int j3 = 8 * (flag1 ? -1 : 1);
               AbstractGui.blit(pPoseStack, k2, l2, 8, 8, 8.0F, (float)i3, 8, j3, 64, 64);
               if (playerentity != null && playerentity.isModelPartShown(PlayerModelPart.HAT)) {
                  int k3 = 8 + (flag1 ? 8 : 0);
                  int l3 = 8 * (flag1 ? -1 : 1);
                  AbstractGui.blit(pPoseStack, k2, l2, 8, 8, 40.0F, (float)k3, 8, l3, 64, 64);
               }

               k2 += 9;
            }

            this.minecraft.font.drawShadow(pPoseStack, this.getNameForDisplay(networkplayerinfo1), (float)k2, (float)l2, networkplayerinfo1.getGameMode() == GameType.SPECTATOR ? -1862270977 : -1);
            if (pObjective != null && networkplayerinfo1.getGameMode() != GameType.SPECTATOR) {
               int l5 = k2 + i + 1;
               int i6 = l5 + l;
               if (i6 - l5 > 5) {
                  this.renderTablistScore(pObjective, l2, gameprofile.getName(), l5, i6, networkplayerinfo1, pPoseStack);
               }
            }

            this.renderPingIcon(pPoseStack, i1, k2 - (flag ? 9 : 0), l2, networkplayerinfo1);
         }
      }

      if (list2 != null) {
         k1 = k1 + j4 * 9 + 1;
         fill(pPoseStack, p_238523_2_ / 2 - l1 / 2 - 1, k1 - 1, p_238523_2_ / 2 + l1 / 2 + 1, k1 + list2.size() * 9, Integer.MIN_VALUE);

         for(IReorderingProcessor ireorderingprocessor3 : list2) {
            int k5 = this.minecraft.font.width(ireorderingprocessor3);
            this.minecraft.font.drawShadow(pPoseStack, ireorderingprocessor3, (float)(p_238523_2_ / 2 - k5 / 2), (float)k1, -1);
            k1 += 9;
         }
      }
   ...
   protected void renderPingIcon(MatrixStack pPoseStack, int p_238522_2_, int p_238522_3_, int p_238522_4_, NetworkPlayerInfo pPlayerInfo) {
      RenderSystem.color4f(1.0F, 1.0F, 1.0F, 1.0F);
      ...
      this.blit(pPoseStack, p_238522_3_ + p_238522_2_ - 11, p_238522_4_, 0, 176 + j * 8, 10, 8);
      this.setBlitOffset(this.getBlitOffset() - 100);
   }

   private void renderTablistScore(ScoreObjective pObjective, int p_175247_2_, String p_175247_3_, int p_175247_4_, int p_175247_5_, NetworkPlayerInfo pPlayerInfo, MatrixStack pPoseStack) {
      int i = pObjective.getScoreboard().getOrCreatePlayerScore(p_175247_3_, pObjective).getScore();
      if (pObjective.getRenderType() == ScoreCriteria.RenderType.HEARTS) {
         this.minecraft.getTextureManager().bind(GUI_ICONS_LOCATION);
         ...
         pPlayerInfo.setLastHealth(i);
         int k = MathHelper.ceil((float)Math.max(i, pPlayerInfo.getDisplayHealth()) / 2.0F);
         int l = Math.max(MathHelper.ceil((float)(i / 2)), Math.max(MathHelper.ceil((float)(pPlayerInfo.getDisplayHealth() / 2)), 10));
         ...
         if (k > 0) {
            ...
            if (i1 > 3) {
               for(int j1 = k; j1 < l; ++j1) {
                  this.blit(pPoseStack, p_175247_4_ + j1 * i1, p_175247_2_, flag ? 25 : 16, 0, 9, 9);
               }

               for(int l1 = 0; l1 < k; ++l1) {
                  this.blit(pPoseStack, p_175247_4_ + l1 * i1, p_175247_2_, flag ? 25 : 16, 0, 9, 9);
                  if (flag) {
                     if (l1 * 2 + 1 < pPlayerInfo.getDisplayHealth()) {
                        this.blit(pPoseStack, p_175247_4_ + l1 * i1, p_175247_2_, 70, 0, 9, 9);
                     }

                     if (l1 * 2 + 1 == pPlayerInfo.getDisplayHealth()) {
                        this.blit(pPoseStack, p_175247_4_ + l1 * i1, p_175247_2_, 79, 0, 9, 9);
                     }
                  }

                  if (l1 * 2 + 1 < i) {
                     this.blit(pPoseStack, p_175247_4_ + l1 * i1, p_175247_2_, l1 >= 10 ? 160 : 52, 0, 9, 9);
                  }

                  if (l1 * 2 + 1 == i) {
                     this.blit(pPoseStack, p_175247_4_ + l1 * i1, p_175247_2_, l1 >= 10 ? 169 : 61, 0, 9, 9);
                  }
               }
            } else {
               ...
               this.minecraft.font.drawShadow(pPoseStack, s, (float)((p_175247_5_ + p_175247_4_) / 2 - this.minecraft.font.width(s) / 2), (float)p_175247_2_, k1);
            }
         }
      } else {
         String s1 = TextFormatting.YELLOW + "" + i;
         this.minecraft.font.drawShadow(pPoseStack, s1, (float)(p_175247_5_ - this.minecraft.font.width(s1)), (float)p_175247_2_, 16777215);
      }

   }

   public void setFooter(@Nullable ITextComponent pFooter) {
      this.footer = pFooter;
   }

   public void setHeader(@Nullable ITextComponent pHeader) {
      this.header = pHeader;
   }
