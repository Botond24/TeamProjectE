...
public class RecipeToast implements IToast {
   ...
   public RecipeToast(IRecipe<?> pRecipe) {
   ...
   public IToast.Visibility render(MatrixStack pPoseStack, ToastGui pToastComponent, long p_230444_3_) {
      ...
      if (this.recipes.isEmpty()) {
         ...
         pToastComponent.getMinecraft().getItemRenderer().renderAndDecorateFakeItem(irecipe.getResultItem(), 8, 8);
         return p_230444_3_ - this.lastChanged >= 5000L ? IToast.Visibility.HIDE : IToast.Visibility.SHOW;
      }
   }

   private void addItem(IRecipe<?> pRecipe) {
      this.recipes.add(pRecipe);
      this.changed = true;
   }

   public static void addOrUpdate(ToastGui pToastGui, IRecipe<?> pRecipe) {
