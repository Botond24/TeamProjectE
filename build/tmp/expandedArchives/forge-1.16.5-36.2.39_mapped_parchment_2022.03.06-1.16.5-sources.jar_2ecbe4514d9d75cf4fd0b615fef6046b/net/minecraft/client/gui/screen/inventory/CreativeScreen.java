...
public class CreativeScreen extends DisplayEffectsScreen<CreativeScreen.CreativeContainer> {
   ...
   public CreativeScreen(PlayerEntity pPlayer) {
      super(new CreativeScreen.CreativeContainer(pPlayer), pPlayer.inventory, StringTextComponent.EMPTY);
   ...
   protected void slotClicked(@Nullable Slot pSlot, int pSlotId, int pMouseButton, ClickType pType) {
      ...
      if (pSlot == null && selectedTab != ItemGroup.TAB_INVENTORY.getId() && pType != ClickType.QUICK_CRAFT) {
         ...
         if (pSlot == this.destroyItemSlot && flag) {
            ...
            if (pSlot == this.destroyItemSlot) {
               ...
               this.minecraft.gameMode.handleCreativeModeItemAdd(itemstack1, ((CreativeScreen.CreativeSlot)pSlot).target.index);
            } else if (pType == ClickType.THROW && !this.minecraft.player.inventory.getCarried().isEmpty()) {
               ...
               this.minecraft.player.inventoryMenu.clicked(pSlot == null ? pSlotId : ((CreativeScreen.CreativeSlot)pSlot).target.index, pMouseButton, pType, this.minecraft.player);
            ...
            if (pType == ClickType.SWAP) {
               if (!itemstack7.isEmpty()) {
                  ...
                  this.minecraft.player.inventory.setItem(pMouseButton, itemstack10);
                  this.minecraft.player.inventoryMenu.broadcastChanges();
               }
            ...
            if (pType == ClickType.CLONE) {
               if (playerinventory.getCarried().isEmpty() && pSlot.hasItem()) {
            ...
            if (Container.getQuickcraftHeader(pMouseButton) == 2) {
               for(int k = 0; k < 9; ++k) {
                  this.minecraft.gameMode.handleCreativeModeItemAdd(this.menu.getSlot(45 + k).getItem(), 36 + k);
               }
            } else if (pSlot != null) {
               ItemStack itemstack4 = this.menu.getSlot(pSlot.index).getItem();
               this.minecraft.gameMode.handleCreativeModeItemAdd(itemstack4, pSlot.index - (this.menu).slots.size() + 9 + 36);
   ...
   private void updateVisibleTags(String pSearch) {
      ...
      if (i == -1) {
         predicate = (p_214084_1_) -> {
            return p_214084_1_.getPath().contains(pSearch);
         };
      } else {
   ...
   protected void renderLabels(MatrixStack pMatrixStack, int pX, int pY) {
      ...
      if (itemgroup != null && itemgroup.showTitle()) {
         ...
         this.font.draw(pMatrixStack, itemgroup.getDisplayName(), 8.0F, 6.0F, itemgroup.getLabelColor());
      }
   ...
   public boolean mouseClicked(double pMouseX, double pMouseY, int pButton) {
      if (pButton == 0) {
         ...
         if (selectedTab != ItemGroup.TAB_INVENTORY.getId() && this.insideScrollbar(pMouseX, pMouseY)) {
    ...
    */
   private void selectTab(ItemGroup pTab) {
      if (pTab == null) return;
      int i = selectedTab;
      ...
      } else if (pTab != ItemGroup.TAB_SEARCH) {
         pTab.fillItemList((this.menu).items);
      }

      if (pTab == ItemGroup.TAB_INVENTORY) {
   ...
   protected boolean hasClickedOutside(double pMouseX, double pMouseY, int pGuiLeft, int pGuiTop, int pMouseButton) {
      ...
      this.hasClickedOutside = flag && !this.checkTabClicked(ItemGroup.TABS[selectedTab], pMouseX, pMouseY);
   ...
   public void render(MatrixStack pMatrixStack, int pMouseX, int pMouseY, float pPartialTicks) {
      ...
      boolean rendered = false;
       ...
       for (int x = start; x < end; x++) {
         ...
         if (itemgroup != null && this.checkTabHovering(pMatrixStack, itemgroup, pMouseX, pMouseY)) {
            rendered = true;
            break;
         }
      }
      if (!rendered && !this.checkTabHovering(pMatrixStack, ItemGroup.TAB_SEARCH, pMouseX, pMouseY))
         this.checkTabHovering(pMatrixStack, ItemGroup.TAB_INVENTORY, pMouseX, pMouseY);

      if (this.destroyItemSlot != null && selectedTab == ItemGroup.TAB_INVENTORY.getId() && this.isHovering(this.destroyItemSlot.x, this.destroyItemSlot.y, 16, 16, (double)pMouseX, (double)pMouseY)) {
      ...
      if (maxPages != 0) {
          ...
          font.drawShadow(pMatrixStack, page.getVisualOrderText(), leftPos + (imageWidth / 2) - (font.width(page) / 2), topPos - 44, -1);
   ...
   protected void renderTooltip(MatrixStack pMatrixStack, ItemStack pItemStack, int pMouseX, int pMouseY) {
      if (selectedTab == ItemGroup.TAB_SEARCH.getId()) {
         List<ITextComponent> list = pItemStack.getTooltipLines(this.minecraft.player, this.minecraft.options.advancedItemTooltips ? ITooltipFlag.TooltipFlags.ADVANCED : ITooltipFlag.TooltipFlags.NORMAL);
         ...
         net.minecraft.client.gui.FontRenderer font = pItemStack.getItem().getFontRenderer(pItemStack);
         net.minecraftforge.fml.client.gui.GuiUtils.preItemToolTip(pItemStack);
         this.renderWrappedToolTip(pMatrixStack, list1, pMouseX, pMouseY, (font == null ? this.font : font));
   ...
   protected void renderBg(MatrixStack pMatrixStack, float pPartialTicks, int pX, int pY) {
      ...
      for (int idx = start; idx < end; idx++) {
         ...
         if (itemgroup1 != null && itemgroup1.getId() != selectedTab) {
            ...
            this.renderTabButton(pMatrixStack, itemgroup1);
         }
      }
      ...
      this.renderTabButton(pMatrixStack, itemgroup);
      if (itemgroup == ItemGroup.TAB_INVENTORY) {
   ...
   protected boolean checkTabClicked(ItemGroup pCreativeModeTab, double pRelativeMouseX, double pRelativeMouseY) {
      if (pCreativeModeTab.getTabPage() != tabPage && pCreativeModeTab != ItemGroup.TAB_SEARCH && pCreativeModeTab != ItemGroup.TAB_INVENTORY) return false;
   ...
   protected boolean checkTabHovering(MatrixStack pPoseStack, ItemGroup pCreativeModeTab, int pMouseX, int pMouseY) {
      int i = pCreativeModeTab.getColumn();
      int j = 28 * i;
      int k = 0;
      if (pCreativeModeTab.isAlignedRight()) {
   ...
   protected void renderTabButton(MatrixStack pPoseStack, ItemGroup pCreativeModeTab) {
   ...
   public static void handleHotbarLoadOrSave(Minecraft pClient, int pIndex, boolean pLoad, boolean pSave) {
      ClientPlayerEntity clientplayerentity = pClient.player;
      CreativeSettings creativesettings = pClient.getHotbarManager();
      HotbarSnapshot hotbarsnapshot = creativesettings.get(pIndex);
      if (pLoad) {
         ...
         ITextComponent itextcomponent = pClient.options.keyHotbarSlots[pIndex].getTranslatedKeyMessage();
         ITextComponent itextcomponent1 = pClient.options.keyLoadHotbarActivator.getTranslatedKeyMessage();
         pClient.gui.setOverlayMessage(new TranslationTextComponent("inventory.hotbarSaved", itextcomponent1, itextcomponent), false);
   ...
   public static class CreativeContainer extends Container {
      ...
      public CreativeContainer(PlayerEntity pPlayer) {
      ...
      public boolean canTakeItemForPickAll(ItemStack pStack, Slot pSlot) {
         return pSlot.container != CreativeScreen.CONTAINER;
      }
   ...
   static class CreativeSlot extends Slot {
      ...
      public CreativeSlot(Slot pSlot, int pIndex, int pX, int pY) {
         super(pSlot.container, pIndex, pX, pY);
         this.target = pSlot;
      }

      public ItemStack onTake(PlayerEntity pPlayer, ItemStack pStack) {
         return this.target.onTake(pPlayer, pStack);
      }
