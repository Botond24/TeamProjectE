...
public class CommandBlockScreen extends AbstractCommandBlockScreen {
   ...
   public CommandBlockScreen(CommandBlockTileEntity pCommandBlockEntity) {
      this.autoCommandBlock = pCommandBlockEntity;
   }
   ...
   protected void populateAndSendPacket(CommandBlockLogic pCommandBlock) {
      this.minecraft.getConnection().send(new CUpdateCommandBlockPacket(new BlockPos(pCommandBlock.getPosition()), this.commandEdit.getValue(), this.mode, pCommandBlock.isTrackOutput(), this.conditional, this.autoexec));
   }
