...
public class RecipeTabToggleWidget extends ToggleWidget {
   ...
   public RecipeTabToggleWidget(RecipeBookCategories p_i51075_1_) {
      ...
      this.initTextureValues(153, 2, 35, 0, RecipeBookGui.RECIPE_BOOK_LOCATION);
   }

   public void startAnimation(Minecraft p_193918_1_) {
      ClientRecipeBook clientrecipebook = p_193918_1_.player.getRecipeBook();
      List<RecipeList> list = clientrecipebook.getCollection(this.category);
      if (p_193918_1_.player.containerMenu instanceof RecipeBookContainer) {
         for(RecipeList recipelist : list) {
            for(IRecipe<?> irecipe : recipelist.getRecipes(clientrecipebook.isFiltering((RecipeBookContainer)p_193918_1_.player.containerMenu))) {
               if (clientrecipebook.willHighlight(irecipe)) {
   ...
   public boolean updateVisibility(ClientRecipeBook p_199500_1_) {
      List<RecipeList> list = p_199500_1_.getCollection(this.category);
      this.visible = false;
      if (list != null) {
         for(RecipeList recipelist : list) {
            if (recipelist.hasKnownRecipes() && recipelist.hasFitting()) {
               this.visible = true;
               break;
            }
         }
