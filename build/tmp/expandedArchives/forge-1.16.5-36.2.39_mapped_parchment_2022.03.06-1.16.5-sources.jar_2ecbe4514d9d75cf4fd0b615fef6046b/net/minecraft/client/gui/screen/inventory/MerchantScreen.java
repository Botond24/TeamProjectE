...
public class MerchantScreen extends ContainerScreen<MerchantContainer> {
   ...
   public MerchantScreen(MerchantContainer pMerchantMenu, PlayerInventory pPlayerInventory, ITextComponent pTitle) {
      super(pMerchantMenu, pPlayerInventory, pTitle);
      this.imageWidth = 276;
   ...
   protected void renderLabels(MatrixStack pMatrixStack, int pX, int pY) {
      ...
      if (i > 0 && i <= 5 && this.menu.showProgressBar()) {
         ...
         this.font.draw(pMatrixStack, itextcomponent, (float)k, 6.0F, 4210752);
      } else {
   ...
   private void renderProgressBar(MatrixStack pPoseStack, int pPosX, int pPosY, MerchantOffer pMerchantOffer) {
      this.minecraft.getTextureManager().bind(VILLAGER_LOCATION);
      int i = this.menu.getTraderLevel();
      int j = this.menu.getTraderXp();
      if (i < 5) {
         blit(pPoseStack, pPosX + 136, pPosY + 16, this.getBlitOffset(), 0.0F, 186.0F, 102, 5, 256, 512);
         ...
         if (j >= k && VillagerData.canLevelUp(i)) {
            ...
            blit(pPoseStack, pPosX + 136, pPosY + 16, this.getBlitOffset(), 0.0F, 191.0F, i1 + 1, 5, 256, 512);
            int j1 = this.menu.getFutureTraderXp();
            if (j1 > 0) {
               int k1 = Math.min(MathHelper.floor((float)j1 * f), 100 - i1);
               blit(pPoseStack, pPosX + 136 + i1 + 1, pPosY + 16 + 1, this.getBlitOffset(), 2.0F, 182.0F, k1, 3, 256, 512);
            }
   ...
   private void renderScroller(MatrixStack pPoseStack, int pPosX, int pPosY, MerchantOffers pMerchantOffers) {
      ...
      if (i > 1) {
         ...
         blit(pPoseStack, pPosX + 94, pPosY + 18 + i1, this.getBlitOffset(), 0.0F, 199.0F, 6, 27, 256, 512);
      } else {
         blit(pPoseStack, pPosX + 94, pPosY + 18, this.getBlitOffset(), 6.0F, 199.0F, 6, 27, 256, 512);
      }
      ...
      if (!merchantoffers.isEmpty()) {
         ...
         for(MerchantScreen.TradeButton merchantscreen$tradebutton : this.tradeOfferButtons) {
            if (merchantscreen$tradebutton.isHovered()) {
               merchantscreen$tradebutton.renderToolTip(pMatrixStack, pMouseX, pMouseY);
            }
   ...
   private void renderButtonArrows(MatrixStack pPoseStack, MerchantOffer pMerchantOffer, int pPosX, int pPosY) {
   ...
   private void renderAndDecorateCostA(MatrixStack pPoseStack, ItemStack pRealCost, ItemStack pBaseCost, int pX, int pY) {
   ...
   class TradeButton extends Button {
      ...
      public TradeButton(int pX, int pY, int pIndex, Button.IPressable pOnPress) {
         super(pX, pY, 89, 20, StringTextComponent.EMPTY, pOnPress);
         this.index = pIndex;
         this.visible = false;
      }
