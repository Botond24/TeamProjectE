...
public class FilterListEntry extends AbstractOptionList.Entry<FilterListEntry> {
   ...
   public void render(MatrixStack pMatrixStack, int pIndex, int pTop, int pLeft, int pWidth, int pHeight, int pMouseX, int pMouseY, boolean pIsMouseOver, float pPartialTicks) {
      ...
      if (itextcomponent == StringTextComponent.EMPTY) {
         AbstractGui.fill(pMatrixStack, pLeft, pTop, pLeft + pWidth, pTop + pHeight, BG_FILL);
         l = pTop + (pHeight - 9) / 2;
      } else {
         AbstractGui.fill(pMatrixStack, pLeft, pTop, pLeft + pWidth, pTop + pHeight, BG_FILL_REMOVED);
         l = pTop + (pHeight - (9 + 9)) / 2;
         this.minecraft.font.draw(pMatrixStack, itextcomponent, (float)k, (float)(l + 12), PLAYER_STATUS_COLOR);
      }

      this.minecraft.getTextureManager().bind(this.skinGetter.get());
      AbstractGui.blit(pMatrixStack, i, j, 24, 24, 8.0F, 8.0F, 8, 8, 64, 64);
      RenderSystem.enableBlend();
      AbstractGui.blit(pMatrixStack, i, j, 24, 24, 40.0F, 8.0F, 8, 8, 64, 64);
      RenderSystem.disableBlend();
      this.minecraft.font.draw(pMatrixStack, this.playerName, (float)k, (float)l, PLAYERNAME_COLOR);
      if (this.isRemoved) {
         AbstractGui.fill(pMatrixStack, i, j, i + 24, j + 24, SKIN_SHADE);
      }
