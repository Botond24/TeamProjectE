...
public class FontTexture extends Texture {
   ...
   @Nullable
   public TexturedGlyph add(IGlyphInfo pGlyphInfo) {
      if (pGlyphInfo.isColored() != this.colored) {
         return null;
      } else {
         FontTexture.Entry fonttexture$entry = this.root.insert(pGlyphInfo);
         if (fonttexture$entry != null) {
            this.bind();
            pGlyphInfo.upload(fonttexture$entry.x, fonttexture$entry.y);
            float f = 256.0F;
            float f1 = 256.0F;
            float f2 = 0.01F;
            return new TexturedGlyph(this.normalType, this.seeThroughType, ((float)fonttexture$entry.x + 0.01F) / 256.0F, ((float)fonttexture$entry.x - 0.01F + (float)pGlyphInfo.getPixelWidth()) / 256.0F, ((float)fonttexture$entry.y + 0.01F) / 256.0F, ((float)fonttexture$entry.y - 0.01F + (float)pGlyphInfo.getPixelHeight()) / 256.0F, pGlyphInfo.getLeft(), pGlyphInfo.getRight(), pGlyphInfo.getUp(), pGlyphInfo.getDown());
         } else {
            return null;
         }
      }
