...
public class SubtitleOverlayGui extends AbstractGui implements ISoundEventListener {
   ...
   public SubtitleOverlayGui(Minecraft pMinecraft) {
   ...
   public void render(MatrixStack pPoseStack) {
      ...
      if (this.isListening && !this.subtitles.isEmpty()) {
         ...
         for(SubtitleOverlayGui.Subtitle subtitleoverlaygui$subtitle1 : this.subtitles) {
            ...
            this.minecraft.font.draw(pPoseStack, itextcomponent, (float)(-k1 / 2), (float)(-j1), i2 + -16777216);
   ...
   public void onPlaySound(ISound pSound, SoundEventAccessor pAccessor) {
      if (pAccessor.getSubtitle() != null) {
         ITextComponent itextcomponent = pAccessor.getSubtitle();
         if (!this.subtitles.isEmpty()) {
            for(SubtitleOverlayGui.Subtitle subtitleoverlaygui$subtitle : this.subtitles) {
               if (subtitleoverlaygui$subtitle.getText().equals(itextcomponent)) {
                  subtitleoverlaygui$subtitle.refresh(new Vector3d(pSound.getX(), pSound.getY(), pSound.getZ()));
                  return;
               }
            }
         }

         this.subtitles.add(new SubtitleOverlayGui.Subtitle(itextcomponent, new Vector3d(pSound.getX(), pSound.getY(), pSound.getZ())));
      }
   }
   ...
   public class Subtitle {
      ...
      public void refresh(Vector3d pLocation) {
