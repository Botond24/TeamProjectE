...
public class FontResourceManager implements AutoCloseable {
   ...
   private final IFutureReloadListener reloadListener = new ReloadListener<Map<ResourceLocation, List<IGlyphProvider>>>() {
       ...
       */
      protected Map<ResourceLocation, List<IGlyphProvider>> prepare(IResourceManager pResourceManager, IProfiler pProfiler) {
         pProfiler.startTick();
         Gson gson = (new GsonBuilder()).setPrettyPrinting().disableHtmlEscaping().create();
         Map<ResourceLocation, List<IGlyphProvider>> map = Maps.newHashMap();

         for(ResourceLocation resourcelocation : pResourceManager.listResources("font", (p_215274_0_) -> {
            ...
            try {
               for(IResource iresource : pResourceManager.getResources(resourcelocation)) {
                  ...
                  try (
                     ...
                     for(int i = jsonarray.size() - 1; i >= 0; --i) {
                        ...
                        try {
                           ...
                           pProfiler.push(s1);
                           IGlyphProvider iglyphprovider = glyphprovidertypes.create(jsonobject).create(pResourceManager);
      ...
      protected void apply(Map<ResourceLocation, List<IGlyphProvider>> pObject, IResourceManager pResourceManager, IProfiler pProfiler) {
         pProfiler.startTick();
         ...
         pObject.forEach((p_238556_1_, p_238556_2_) -> {
