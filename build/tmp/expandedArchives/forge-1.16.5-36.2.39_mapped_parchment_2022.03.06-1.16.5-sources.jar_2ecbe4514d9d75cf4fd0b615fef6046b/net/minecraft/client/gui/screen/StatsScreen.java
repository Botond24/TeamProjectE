...
public class StatsScreen extends Screen implements IProgressMeter {
   ...
   public StatsScreen(Screen pLastScreen, StatisticsManager pStats) {
   ...
   class CustomStatsList extends ExtendedList<StatsScreen.CustomStatsList.Entry> {
      ...
      class Entry extends ExtendedList.AbstractListEntry<StatsScreen.CustomStatsList.Entry> {
         ...
         public void render(MatrixStack pMatrixStack, int pIndex, int pTop, int pLeft, int pWidth, int pHeight, int pMouseX, int pMouseY, boolean pIsMouseOver, float pPartialTicks) {
            AbstractGui.drawString(pMatrixStack, StatsScreen.this.font, this.statDisplay, pLeft + 2, pTop + 1, pIndex % 2 == 0 ? 16777215 : 9474192);
            String s = this.stat.format(StatsScreen.this.stats.getValue(this.stat));
            AbstractGui.drawString(pMatrixStack, StatsScreen.this.font, s, pLeft + 2 + 213 - StatsScreen.this.font.width(s), pTop + 1, pIndex % 2 == 0 ? 16777215 : 9474192);
         }
      }
   }
   ...
   class MobStatsList extends ExtendedList<StatsScreen.MobStatsList.Entry> {
      ...
      class Entry extends ExtendedList.AbstractListEntry<StatsScreen.MobStatsList.Entry> {
         ...
         public void render(MatrixStack pMatrixStack, int pIndex, int pTop, int pLeft, int pWidth, int pHeight, int pMouseX, int pMouseY, boolean pIsMouseOver, float pPartialTicks) {
            AbstractGui.drawString(pMatrixStack, StatsScreen.this.font, this.mobName, pLeft + 2, pTop + 1, 16777215);
            AbstractGui.drawString(pMatrixStack, StatsScreen.this.font, this.kills, pLeft + 2 + 10, pTop + 1 + 9, this.hasKills ? 9474192 : 6316128);
            AbstractGui.drawString(pMatrixStack, StatsScreen.this.font, this.killedBy, pLeft + 2 + 10, pTop + 1 + 9 * 2, this.wasKilledBy ? 9474192 : 6316128);
         }
      }
   }
   ...
   class StatsList extends ExtendedList<StatsScreen.StatsList.Entry> {
      ...
      protected void renderDecorations(MatrixStack pPoseStack, int pMouseX, int pMouseY) {
         if (pMouseY >= this.y0 && pMouseY <= this.y1) {
            StatsScreen.StatsList.Entry statsscreen$statslist$entry = this.getEntryAtPosition((double)pMouseX, (double)pMouseY);
            ...
            if (statsscreen$statslist$entry != null) {
               ...
               this.renderMousehoverTooltip(pPoseStack, itextcomponent, pMouseX, pMouseY);
            }
      ...
      class Entry extends ExtendedList.AbstractListEntry<StatsScreen.StatsList.Entry> {
         ...
         public void render(MatrixStack pMatrixStack, int pIndex, int pTop, int pLeft, int pWidth, int pHeight, int pMouseX, int pMouseY, boolean pIsMouseOver, float pPartialTicks) {
            Item item = StatsScreen.this.itemStatsList.statItemList.get(pIndex);
            StatsScreen.this.blitSlot(pMatrixStack, pLeft + 40, pTop, item);
            ...
            for(int j = 0; j < StatsScreen.this.itemStatsList.itemColumns.size(); ++j) {
               this.renderStat(pMatrixStack, StatsScreen.this.itemStatsList.itemColumns.get(j).get(item), pLeft + StatsScreen.this.getColumnX(j + StatsScreen.this.itemStatsList.blockColumns.size()), pTop, pIndex % 2 == 0);
            }
