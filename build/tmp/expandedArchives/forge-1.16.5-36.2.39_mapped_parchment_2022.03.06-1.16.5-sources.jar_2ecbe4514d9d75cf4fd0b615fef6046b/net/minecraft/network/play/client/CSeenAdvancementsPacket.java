...
public class CSeenAdvancementsPacket implements IPacket<IServerPlayNetHandler> {
   ...
   @OnlyIn(Dist.CLIENT)
   public CSeenAdvancementsPacket(CSeenAdvancementsPacket.Action pAction, @Nullable ResourceLocation pTab) {
      this.action = pAction;
      this.tab = pTab;
   }

   @OnlyIn(Dist.CLIENT)
   public static CSeenAdvancementsPacket openedTab(Advancement pAdvancement) {
      return new CSeenAdvancementsPacket(CSeenAdvancementsPacket.Action.OPENED_TAB, pAdvancement.getId());
   }
