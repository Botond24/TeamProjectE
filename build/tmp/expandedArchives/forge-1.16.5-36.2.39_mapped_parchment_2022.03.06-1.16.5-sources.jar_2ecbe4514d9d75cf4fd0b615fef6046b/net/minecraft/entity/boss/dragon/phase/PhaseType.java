...
public class PhaseType<T extends IPhase> {
   ...
   private PhaseType(int pId, Class<? extends IPhase> pInstanceClass, String pName) {
      this.id = pId;
      this.instanceClass = pInstanceClass;
      this.name = pName;
   }

   public IPhase createInstance(EnderDragonEntity pDragon) {
      try {
    ...
    */
   public static PhaseType<?> getById(int pId) {
   ...
   private static <T extends IPhase> PhaseType<T> create(Class<T> pPhase, String pName) {
      PhaseType<T> phasetype = new PhaseType<>(phases.length, pPhase, pName);
