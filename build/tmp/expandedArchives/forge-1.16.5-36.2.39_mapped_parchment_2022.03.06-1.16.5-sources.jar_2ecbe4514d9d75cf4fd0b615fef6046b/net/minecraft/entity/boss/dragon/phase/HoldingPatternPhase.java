...
public class HoldingPatternPhase extends Phase {
   ...
   private void strafePlayer(PlayerEntity pPlayer) {
      this.dragon.getPhaseManager().setPhase(PhaseType.STRAFE_PLAYER);
      this.dragon.getPhaseManager().getPhase(PhaseType.STRAFE_PLAYER).setTarget(pPlayer);
   }
   ...
   public void onCrystalDestroyed(EnderCrystalEntity pCrystal, BlockPos pPos, DamageSource pDmgSrc, @Nullable PlayerEntity pPlyr) {
      if (pPlyr != null && !pPlyr.abilities.invulnerable) {
