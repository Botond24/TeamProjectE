...
public class EvokerEntity extends SpellcastingIllagerEntity {
   ...
   private void setWololoTarget(@Nullable SheepEntity pWololoTarget) {
   ...
   class AttackSpellGoal extends SpellcastingIllagerEntity.UseSpellGoal {
      ...
      private void createSpellEntity(double pX, double pZ, double pMinY, double pMaxY, float pYRot, int pWarmupDelay) {
         ...
         } while(blockpos.getY() >= MathHelper.floor(pMinY) - 1);

         if (flag) {
            EvokerEntity.this.level.addFreshEntity(new EvokerFangsEntity(EvokerEntity.this.level, pX, (double)blockpos.getY() + d0, pZ, pYRot, pWarmupDelay, EvokerEntity.this));
         }
