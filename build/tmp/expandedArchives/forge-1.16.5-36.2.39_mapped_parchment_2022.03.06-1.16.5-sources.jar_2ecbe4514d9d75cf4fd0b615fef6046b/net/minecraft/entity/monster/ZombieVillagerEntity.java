...
public class ZombieVillagerEntity extends ZombieEntity implements IVillagerDataHolder {
   ...
   private void startConverting(@Nullable UUID pConversionStarter, int pVillagerConversionTime) {
      ...
      this.addEffect(new EffectInstance(Effects.DAMAGE_BOOST, pVillagerConversionTime, Math.min(this.level.getDifficulty().getId() - 1, 0)));
   ...
   private void finishConversion(ServerWorld pServerLevel) {
      ...
      villagerentity.finalizeSpawn(pServerLevel, pServerLevel.getCurrentDifficultyAt(villagerentity.blockPosition()), SpawnReason.CONVERSION, (ILivingEntityData)null, (CompoundNBT)null);
      if (this.conversionStarter != null) {
         PlayerEntity playerentity = pServerLevel.getPlayerByUUID(this.conversionStarter);
         if (playerentity instanceof ServerPlayerEntity) {
            CriteriaTriggers.CURED_ZOMBIE_VILLAGER.trigger((ServerPlayerEntity)playerentity, this, villagerentity);
            pServerLevel.onReputationEvent(IReputationType.ZOMBIE_VILLAGER_CURED, playerentity, villagerentity);
         }
      }

      villagerentity.addEffect(new EffectInstance(Effects.CONFUSION, 200, 0));
      if (!this.isSilent()) {
         pServerLevel.levelEvent((PlayerEntity)null, 1027, this.blockPosition(), 0);
      }
   ...
   public void setTradeOffers(CompoundNBT pTradeOffers) {
      this.tradeOffers = pTradeOffers;
   }

   public void setGossips(INBT pGossips) {
