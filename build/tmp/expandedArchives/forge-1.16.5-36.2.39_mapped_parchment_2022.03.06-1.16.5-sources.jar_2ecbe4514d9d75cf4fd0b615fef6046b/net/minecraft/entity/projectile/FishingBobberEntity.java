...
public class FishingBobberEntity extends ProjectileEntity {
   ...
   private FishingBobberEntity(World pLevel, PlayerEntity pPlayer, int pLuck, int pLureSpeed) {
      super(EntityType.FISHING_BOBBER, pLevel);
      this.noCulling = true;
      this.setOwner(pPlayer);
      pPlayer.fishing = this;
      this.luck = Math.max(0, pLuck);
      this.lureSpeed = Math.max(0, pLureSpeed);
   }

   @OnlyIn(Dist.CLIENT)
   public FishingBobberEntity(World pLevel, PlayerEntity pPlayer, double pX, double pY, double pZ) {
      this(pLevel, pPlayer, 0, 0);
   ...
   public FishingBobberEntity(PlayerEntity pPlayer, World pLevel, int pLuck, int pLureSpeed) {
      this(pLevel, pPlayer, pLuck, pLureSpeed);
   ...
   private boolean shouldStopFishing(PlayerEntity pPlayer) {
      ...
      if (!pPlayer.removed && pPlayer.isAlive() && (flag || flag1) && !(this.distanceToSqr(pPlayer) > 1024.0D)) {
         return false;
      } else {
         this.remove();
         return true;
   ...
   protected boolean canHitEntity(Entity pTarget) {
      return super.canHitEntity(pTarget) || pTarget.isAlive() && pTarget instanceof ItemEntity;
   }
   ...
   private boolean calculateOpenWater(BlockPos pPos) {
      ...
      for(int i = -1; i <= 2; ++i) {
         FishingBobberEntity.WaterType fishingbobberentity$watertype1 = this.getOpenWaterTypeForArea(pPos.offset(-2, i, -2), pPos.offset(2, i, 2));
   ...
   private FishingBobberEntity.WaterType getOpenWaterTypeForArea(BlockPos pFirstPos, BlockPos pSecondPos) {
      return BlockPos.betweenClosedStream(pFirstPos, pSecondPos).map(this::getOpenWaterTypeForBlock).reduce((p_234601_0_, p_234601_1_) -> {
   ...
   public int retrieve(ItemStack pStack) {
      ...
      if (!this.level.isClientSide && playerentity != null) {
         ...
         if (this.hookedIn != null) {
            ...
            CriteriaTriggers.FISHING_ROD_HOOKED.trigger((ServerPlayerEntity)playerentity, pStack, this, Collections.emptyList());
            this.level.broadcastEntityEvent(this, (byte)31);
            i = this.hookedIn instanceof ItemEntity ? 3 : 5;
         } else if (this.nibble > 0) {
            LootContext.Builder lootcontext$builder = (new LootContext.Builder((ServerWorld)this.level)).withParameter(LootParameters.ORIGIN, this.position()).withParameter(LootParameters.TOOL, pStack).withParameter(LootParameters.THIS_ENTITY, this).withRandom(this.random).withLuck((float)this.luck + playerentity.getLuck());
            ...
            CriteriaTriggers.FISHING_ROD_HOOKED.trigger((ServerPlayerEntity)playerentity, pStack, this, list);
   ...
   public void handleEntityEvent(byte pId) {
      if (pId == 31 && this.level.isClientSide && this.hookedIn instanceof PlayerEntity && ((PlayerEntity)this.hookedIn).isLocalPlayer()) {
         this.bringInHookedEntity();
      }
