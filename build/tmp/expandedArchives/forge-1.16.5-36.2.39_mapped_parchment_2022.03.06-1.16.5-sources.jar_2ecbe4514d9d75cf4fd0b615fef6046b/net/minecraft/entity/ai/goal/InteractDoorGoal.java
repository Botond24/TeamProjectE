...
public abstract class InteractDoorGoal extends Goal {
   ...
   public InteractDoorGoal(MobEntity pMob) {
      this.mob = pMob;
      if (!GroundPathHelper.hasGroundPathNavigation(pMob)) {
   ...
   protected void setOpen(boolean pOpen) {
      if (this.hasDoor) {
         BlockState blockstate = this.mob.level.getBlockState(this.doorPos);
         if (blockstate.getBlock() instanceof DoorBlock) {
            ((DoorBlock)blockstate.getBlock()).setOpen(this.mob.level, blockstate, this.doorPos, pOpen);
         }
      }
