...
public class LookAtEntityTask extends Task<LivingEntity> {
   ...
   protected boolean checkExtraStartConditions(ServerWorld pLevel, LivingEntity pOwner) {
      return pOwner.getBrain().getMemory(MemoryModuleType.VISIBLE_LIVING_ENTITIES).get().stream().anyMatch(this.predicate);
   }
   ...
   protected void start(ServerWorld pLevel, LivingEntity pEntity, long pGameTime) {
      ...
      brain.getMemory(MemoryModuleType.VISIBLE_LIVING_ENTITIES).ifPresent((p_220515_3_) -> {
         p_220515_3_.stream().filter(this.predicate).filter((p_220517_2_) -> {
            return p_220517_2_.distanceToSqr(pEntity) <= (double)this.maxDistSqr;
