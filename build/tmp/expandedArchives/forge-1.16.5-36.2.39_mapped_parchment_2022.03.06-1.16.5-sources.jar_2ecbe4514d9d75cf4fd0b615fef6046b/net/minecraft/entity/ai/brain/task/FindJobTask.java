...
public class FindJobTask extends Task<VillagerEntity> {
   ...
   protected void start(ServerWorld pLevel, VillagerEntity pEntity, long pGameTime) {
      ...
      Optional<PointOfInterestType> optional = pLevel.getPoiManager().getType(blockpos);
      if (optional.isPresent()) {
         BrainUtil.getNearbyVillagersWithCondition(pEntity, (p_234021_3_) -> {
            return this.nearbyWantsJobsite(optional.get(), p_234021_3_, blockpos);
         }).findFirst().ifPresent((p_234023_4_) -> {
            this.yieldJobSite(pLevel, pEntity, p_234023_4_, blockpos, p_234023_4_.getBrain().getMemory(MemoryModuleType.JOB_SITE).isPresent());
         });
      }
   }
