...
public class StayNearPointTask extends Task<VillagerEntity> {
   ...
   protected void start(ServerWorld pLevel, VillagerEntity pEntity, long pGameTime) {
      ...
      brain.getMemory(this.memoryType).ifPresent((p_220545_6_) -> {
         if (!this.wrongDimension(pLevel, p_220545_6_) && !this.tiredOfTryingToFindTarget(pLevel, pEntity)) {
            if (this.tooFar(pEntity, p_220545_6_)) {
               Vector3d vector3d = null;
               int i = 0;

               for(int j = 1000; i < 1000 && (vector3d == null || this.tooFar(pEntity, GlobalPos.of(pLevel.dimension(), new BlockPos(vector3d)))); ++i) {
                  vector3d = RandomPositionGenerator.getPosTowards(pEntity, 15, 7, Vector3d.atBottomCenterOf(p_220545_6_.pos()));
               }
            ...
            } else if (!this.closeEnough(pLevel, pEntity, p_220545_6_)) {
