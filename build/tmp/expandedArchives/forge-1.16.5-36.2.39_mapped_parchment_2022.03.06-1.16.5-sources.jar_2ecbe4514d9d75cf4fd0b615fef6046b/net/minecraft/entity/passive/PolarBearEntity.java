...
public class PolarBearEntity extends AnimalEntity implements IAngerable {
   ...
   public AgeableEntity getBreedOffspring(ServerWorld pServerLevel, AgeableEntity pMate) {
      return EntityType.POLAR_BEAR.create(pServerLevel);
   }
   ...
   public static boolean checkPolarBearSpawnRules(EntityType<PolarBearEntity> pPolarBear, IWorld pLevel, SpawnReason pSpawnType, BlockPos pPos, Random pRandom) {
      ...
      if (!Objects.equals(optional, Optional.of(Biomes.FROZEN_OCEAN)) && !Objects.equals(optional, Optional.of(Biomes.DEEP_FROZEN_OCEAN))) {
         ...
         return pLevel.getRawBrightness(pPos, 0) > 8 && pLevel.getBlockState(pPos.below()).is(Blocks.ICE);
      }
   }
   ...
   public float getStandingAnimationScale(float pPartialTick) {
      return MathHelper.lerp(pPartialTick, this.clientSideStandAnimationO, this.clientSideStandAnimation) / 6.0F;
   }
