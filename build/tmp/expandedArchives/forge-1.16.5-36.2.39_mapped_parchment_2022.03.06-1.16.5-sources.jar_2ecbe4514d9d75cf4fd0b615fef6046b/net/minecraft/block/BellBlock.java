...
public class BellBlock extends ContainerBlock {
   ...
   public boolean attemptToRing(World pLevel, BlockPos pPos, @Nullable Direction pDirection) {
      ...
      if (!pLevel.isClientSide && tileentity instanceof BellTileEntity) {
         if (pDirection == null) {
            pDirection = pLevel.getBlockState(pPos).getValue(FACING);
         }

         ((BellTileEntity)tileentity).onHit(pDirection);
         pLevel.playSound((PlayerEntity)null, pPos, SoundEvents.BELL_BLOCK, SoundCategory.BLOCKS, 2.0F, 1.0F);
         return true;
      } else {
         return false;
      }
