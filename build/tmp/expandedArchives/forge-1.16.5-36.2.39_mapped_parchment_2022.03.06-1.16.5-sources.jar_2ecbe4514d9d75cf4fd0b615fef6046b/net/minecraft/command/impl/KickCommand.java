...
public class KickCommand {
   public static void register(CommandDispatcher<CommandSource> pDispatcher) {
      pDispatcher.register(Commands.literal("kick").requires((p_198517_0_) -> {
   ...
   private static int kickPlayers(CommandSource pSource, Collection<ServerPlayerEntity> pPlayers, ITextComponent pReason) {
      for(ServerPlayerEntity serverplayerentity : pPlayers) {
         serverplayerentity.connection.disconnect(pReason);
         pSource.sendSuccess(new TranslationTextComponent("commands.kick.success", serverplayerentity.getDisplayName(), pReason), true);
      }
