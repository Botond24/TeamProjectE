...
public class WeatherCommand {
   public static void register(CommandDispatcher<CommandSource> pDispatcher) {
      pDispatcher.register(Commands.literal("weather").requires((p_198868_0_) -> {
   ...
   private static int setClear(CommandSource pSource, int pTime) {
      pSource.getLevel().setWeatherParameters(pTime, 0, false, false);
      pSource.sendSuccess(new TranslationTextComponent("commands.weather.set.clear"), true);
      return pTime;
   }

   private static int setRain(CommandSource pSource, int pTime) {
      pSource.getLevel().setWeatherParameters(0, pTime, true, false);
      pSource.sendSuccess(new TranslationTextComponent("commands.weather.set.rain"), true);
      return pTime;
   }

   private static int setThunder(CommandSource pSource, int pTime) {
      pSource.getLevel().setWeatherParameters(0, pTime, true, true);
      pSource.sendSuccess(new TranslationTextComponent("commands.weather.set.thunder"), true);
      return pTime;
   }
}