...
public class StopSoundCommand {
   public static void register(CommandDispatcher<CommandSource> pDispatcher) {
      ...
      pDispatcher.register(Commands.literal("stopsound").requires((p_198734_0_) -> {
         return p_198734_0_.hasPermission(2);
      }).then(requiredargumentbuilder));
   }

   private static int stopSound(CommandSource pSource, Collection<ServerPlayerEntity> pTargets, @Nullable SoundCategory pCategory, @Nullable ResourceLocation pSound) {
      SStopSoundPacket sstopsoundpacket = new SStopSoundPacket(pSound, pCategory);
      ...
      if (pCategory != null) {
         if (pSound != null) {
            pSource.sendSuccess(new TranslationTextComponent("commands.stopsound.success.source.sound", pSound, pCategory.getName()), true);
         } else {
            pSource.sendSuccess(new TranslationTextComponent("commands.stopsound.success.source.any", pCategory.getName()), true);
         }
      } else if (pSound != null) {
         pSource.sendSuccess(new TranslationTextComponent("commands.stopsound.success.sourceless.sound", pSound), true);
      } else {
         pSource.sendSuccess(new TranslationTextComponent("commands.stopsound.success.sourceless.any"), true);
      }
