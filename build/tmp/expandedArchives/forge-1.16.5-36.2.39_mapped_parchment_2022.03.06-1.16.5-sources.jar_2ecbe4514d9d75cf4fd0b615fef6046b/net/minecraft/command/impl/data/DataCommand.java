...
public class DataCommand {
    ...
    * @return 1
    */
   private static int removeData(CommandSource pSource, IDataAccessor pAccessor, NBTPathArgument.NBTPath pPath) throws CommandSyntaxException {
      CompoundNBT compoundnbt = pAccessor.getData();
      int i = pPath.remove(compoundnbt);
      if (i == 0) {
         throw ERROR_MERGE_UNCHANGED.create();
      } else {
    ...
    * for primitives.
    */
   private static int getData(CommandSource pSource, IDataAccessor pAccessor, NBTPathArgument.NBTPath pPath) throws CommandSyntaxException {
      INBT inbt = getSingleTag(pPath, pAccessor);
      int i;
      ...
      pSource.sendSuccess(pAccessor.getPrintSuccess(inbt), false);
      return i;
   }
    ...
    */
   private static int getNumeric(CommandSource pSource, IDataAccessor pAccessor, NBTPathArgument.NBTPath pPath, double pScale) throws CommandSyntaxException {
      INBT inbt = getSingleTag(pPath, pAccessor);
      if (!(inbt instanceof NumberNBT)) {
         throw ERROR_GET_NOT_NUMBER.create(pPath.toString());
      } else {
         int i = MathHelper.floor(((NumberNBT)inbt).getAsDouble() * pScale);
    ...
    * @return 1
    */
   private static int getData(CommandSource pSource, IDataAccessor pAccessor) throws CommandSyntaxException {
    ...
    * @return 1
    */
   private static int mergeData(CommandSource pSource, IDataAccessor pAccessor, CompoundNBT pNbt) throws CommandSyntaxException {
      CompoundNBT compoundnbt = pAccessor.getData();
      CompoundNBT compoundnbt1 = compoundnbt.copy().merge(pNbt);
      if (compoundnbt.equals(compoundnbt1)) {
         throw ERROR_MERGE_UNCHANGED.create();
      } else {
         pAccessor.setData(compoundnbt1);
         pSource.sendSuccess(pAccessor.getModifiedSuccess(), true);
         return 1;
      }
   }
   ...
   public interface IDataProvider {
       ...
       * createArgument}.
       */
      IDataAccessor access(CommandContext<CommandSource> pContext) throws CommandSyntaxException;

      /**
       * Creates an argument used for accessing data related to this type of thing, including a literal to distinguish
       * from other types.
       */
      ArgumentBuilder<CommandSource, ?> wrap(ArgumentBuilder<CommandSource, ?> pBuilder, Function<ArgumentBuilder<CommandSource, ?>, ArgumentBuilder<CommandSource, ?>> pAction);
   }
