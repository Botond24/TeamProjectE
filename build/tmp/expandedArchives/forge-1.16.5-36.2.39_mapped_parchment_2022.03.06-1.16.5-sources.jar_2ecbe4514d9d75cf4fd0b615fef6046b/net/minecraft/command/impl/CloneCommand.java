...
public class CloneCommand {
   ...
   public static void register(CommandDispatcher<CommandSource> pDispatcher) {
      pDispatcher.register(Commands.literal("clone").requires((p_198271_0_) -> {
   ...
   private static int clone(CommandSource pSource, BlockPos pBeginPos, BlockPos pEndPos, BlockPos pDestPos, Predicate<CachedBlockInfo> pFilterPredicate, CloneCommand.Mode pCloneMode) throws CommandSyntaxException {
      MutableBoundingBox mutableboundingbox = new MutableBoundingBox(pBeginPos, pEndPos);
      BlockPos blockpos = pDestPos.offset(mutableboundingbox.getLength());
      MutableBoundingBox mutableboundingbox1 = new MutableBoundingBox(pDestPos, blockpos);
      if (!pCloneMode.canOverlap() && mutableboundingbox1.intersects(mutableboundingbox)) {
         ...
         if (i > 32768) {
            ...
            if (serverworld.hasChunksAt(pBeginPos, pEndPos) && serverworld.hasChunksAt(pDestPos, blockpos)) {
               ...
               for(int j = mutableboundingbox.z0; j <= mutableboundingbox.z1; ++j) {
                  for(int k = mutableboundingbox.y0; k <= mutableboundingbox.y1; ++k) {
                     for(int l = mutableboundingbox.x0; l <= mutableboundingbox.x1; ++l) {
                        ...
                        if (pFilterPredicate.test(cachedblockinfo)) {
               ...
               if (pCloneMode == CloneCommand.Mode.MOVE) {
               ...
               if (i1 == 0) {
                  ...
                  pSource.sendSuccess(new TranslationTextComponent("commands.clone.success", i1), true);
                  return i1;
               }
            } else {
