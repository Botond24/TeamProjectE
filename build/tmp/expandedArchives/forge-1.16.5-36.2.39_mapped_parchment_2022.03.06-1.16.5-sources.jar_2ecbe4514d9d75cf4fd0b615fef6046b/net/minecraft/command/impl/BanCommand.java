...
public class BanCommand {
   ...
   public static void register(CommandDispatcher<CommandSource> pDispatcher) {
      pDispatcher.register(Commands.literal("ban").requires((p_198238_0_) -> {
   ...
   private static int banPlayers(CommandSource pSource, Collection<GameProfile> pGameProfiles, @Nullable ITextComponent pReason) throws CommandSyntaxException {
      BanList banlist = pSource.getServer().getPlayerList().getBans();
      int i = 0;

      for(GameProfile gameprofile : pGameProfiles) {
         if (!banlist.isBanned(gameprofile)) {
            ProfileBanEntry profilebanentry = new ProfileBanEntry(gameprofile, (Date)null, pSource.getTextName(), (Date)null, pReason == null ? null : pReason.getString());
            banlist.add(profilebanentry);
            ++i;
            pSource.sendSuccess(new TranslationTextComponent("commands.ban.success", TextComponentUtils.getDisplayName(gameprofile), profilebanentry.getReason()), true);
            ServerPlayerEntity serverplayerentity = pSource.getServer().getPlayerList().getPlayer(gameprofile.getId());
