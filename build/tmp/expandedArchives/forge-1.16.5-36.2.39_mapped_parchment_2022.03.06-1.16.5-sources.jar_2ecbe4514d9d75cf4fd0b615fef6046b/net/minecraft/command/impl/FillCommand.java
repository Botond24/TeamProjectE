...
public class FillCommand {
   ...
   public static void register(CommandDispatcher<CommandSource> pDispatcher) {
      pDispatcher.register(Commands.literal("fill").requires((p_198471_0_) -> {
   ...
   private static int fillBlocks(CommandSource pSource, MutableBoundingBox pArea, BlockStateInput pNewBlock, FillCommand.Mode pMode, @Nullable Predicate<CachedBlockInfo> pReplacingPredicate) throws CommandSyntaxException {
      ...
      if (i > 32768) {
         ...
         for(BlockPos blockpos : BlockPos.betweenClosed(pArea.x0, pArea.y0, pArea.z0, pArea.x1, pArea.y1, pArea.z1)) {
            if (pReplacingPredicate == null || pReplacingPredicate.test(new CachedBlockInfo(serverworld, blockpos, true))) {
               BlockStateInput blockstateinput = pMode.filter.filter(pArea, blockpos, pNewBlock, serverworld);
         ...
         if (j == 0) {
            ...
            pSource.sendSuccess(new TranslationTextComponent("commands.fill.success", j), true);
            return j;
         }
      }
   }
