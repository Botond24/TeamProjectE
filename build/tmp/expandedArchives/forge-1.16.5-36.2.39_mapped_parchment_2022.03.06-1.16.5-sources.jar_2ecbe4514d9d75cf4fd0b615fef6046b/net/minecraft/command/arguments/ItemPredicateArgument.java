...
public class ItemPredicateArgument implements ArgumentType<ItemPredicateArgument.IResult> {
   ...
   public static Predicate<ItemStack> getItemPredicate(CommandContext<CommandSource> pContext, String pName) throws CommandSyntaxException {
      return pContext.getArgument(pName, ItemPredicateArgument.IResult.class).create(pContext);
   }
   ...
   static class ItemPredicate implements Predicate<ItemStack> {
      ...
      public ItemPredicate(Item pItem, @Nullable CompoundNBT pTag) {
         this.item = pItem;
         this.nbt = pTag;
      }
   ...
   static class TagPredicate implements Predicate<ItemStack> {
      ...
      public TagPredicate(ITag<Item> pTag, @Nullable CompoundNBT pNbt) {
         this.tag = pTag;
         this.nbt = pNbt;
      }
