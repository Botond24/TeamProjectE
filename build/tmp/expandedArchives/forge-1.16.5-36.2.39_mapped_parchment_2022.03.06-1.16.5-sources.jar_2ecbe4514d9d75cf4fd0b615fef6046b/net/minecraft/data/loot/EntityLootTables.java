...
public class EntityLootTables implements Consumer<BiConsumer<ResourceLocation, LootTable.Builder>> {
   ...
   private static LootTable.Builder createSheepTable(IItemProvider pWool) {
      return LootTable.lootTable().withPool(LootPool.lootPool().setRolls(ConstantRange.exactly(1)).add(ItemLootEntry.lootTableItem(pWool))).withPool(LootPool.lootPool().setRolls(ConstantRange.exactly(1)).add(TableLootEntry.lootTableReference(EntityType.SHEEP.getDefaultLootTable())));
   }
