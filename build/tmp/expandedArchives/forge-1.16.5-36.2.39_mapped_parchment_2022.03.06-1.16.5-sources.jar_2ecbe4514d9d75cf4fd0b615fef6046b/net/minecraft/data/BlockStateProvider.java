...
public class BlockStateProvider implements IDataProvider {
   ...
   public BlockStateProvider(DataGenerator pGenerator) {
   ...
   public void run(DirectoryCache pCache) {
      ...
      if (!list.isEmpty()) {
         ...
         this.saveCollection(pCache, path, map, BlockStateProvider::createBlockStatePath);
         this.saveCollection(pCache, path, map1, BlockStateProvider::createModelPath);
      }
   }

   private <T> void saveCollection(DirectoryCache pCache, Path pRootPath, Map<T, ? extends Supplier<JsonElement>> pObjectToJsonMap, BiFunction<Path, T, Path> pResolveObjectPath) {
      pObjectToJsonMap.forEach((p_240088_3_, p_240088_4_) -> {
         Path path = pResolveObjectPath.apply(pRootPath, p_240088_3_);

         try {
            IDataProvider.save(GSON, pCache, p_240088_4_.get(), path);
