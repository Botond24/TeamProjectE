...
public class HusbandryAdvancements implements Consumer<Consumer<Advancement>> {
   ...
   private Advancement.Builder addFood(Advancement.Builder pBuilder) {
      for(Item item : EDIBLE_ITEMS) {
         pBuilder.addCriterion(Registry.ITEM.getKey(item).getPath(), ConsumeItemTrigger.Instance.usedItem(item));
      }
   ...
   private Advancement.Builder addBreedable(Advancement.Builder pBuilder) {
      for(EntityType<?> entitytype : BREEDABLE_ANIMALS) {
         pBuilder.addCriterion(EntityType.getKey(entitytype).toString(), BredAnimalsTrigger.Instance.bredAnimals(EntityPredicate.Builder.entity().of(entitytype)));
      }

      pBuilder.addCriterion(EntityType.getKey(EntityType.TURTLE).toString(), BredAnimalsTrigger.Instance.bredAnimals(EntityPredicate.Builder.entity().of(EntityType.TURTLE).build(), EntityPredicate.Builder.entity().of(EntityType.TURTLE).build(), EntityPredicate.ANY));
      return pBuilder;
   }

   private Advancement.Builder addFishBuckets(Advancement.Builder pBuilder) {
      for(Item item : FISH_BUCKETS) {
         pBuilder.addCriterion(Registry.ITEM.getKey(item).getPath(), FilledBucketTrigger.Instance.filledBucket(ItemPredicate.Builder.item().of(item).build()));
      }
   ...
   private Advancement.Builder addFish(Advancement.Builder pBuilder) {
      for(Item item : FISH) {
         pBuilder.addCriterion(Registry.ITEM.getKey(item).getPath(), FishingRodHookedTrigger.Instance.fishedItem(ItemPredicate.ANY, EntityPredicate.ANY, ItemPredicate.Builder.item().of(item).build()));
      }
   ...
   private Advancement.Builder addCatVariants(Advancement.Builder pBuilder) {
      CatEntity.TEXTURE_BY_TYPE.forEach((p_218461_1_, p_218461_2_) -> {
         pBuilder.addCriterion(p_218461_2_.getPath(), TameAnimalTrigger.Instance.tamedAnimal(EntityPredicate.Builder.entity().of(p_218461_2_).build()));
      });
      return pBuilder;
   }
}