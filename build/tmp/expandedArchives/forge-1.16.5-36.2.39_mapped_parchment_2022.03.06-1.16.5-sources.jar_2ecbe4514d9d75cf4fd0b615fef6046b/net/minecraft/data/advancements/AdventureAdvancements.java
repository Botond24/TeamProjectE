...
public class AdventureAdvancements implements Consumer<Consumer<Advancement>> {
   ...
   private Advancement.Builder addMobsToKill(Advancement.Builder pBuilder) {
      for(EntityType<?> entitytype : MOBS_TO_KILL) {
         pBuilder.addCriterion(Registry.ENTITY_TYPE.getKey(entitytype).toString(), KilledTrigger.Instance.playerKilledEntity(EntityPredicate.Builder.entity().of(entitytype)));
      }

      return pBuilder;
   }

   protected static Advancement.Builder addBiomes(Advancement.Builder pBuilder, List<RegistryKey<Biome>> pBiomes) {
      for(RegistryKey<Biome> registrykey : pBiomes) {
         pBuilder.addCriterion(registrykey.location().toString(), PositionTrigger.Instance.located(LocationPredicate.inBiome(registrykey)));
      }
