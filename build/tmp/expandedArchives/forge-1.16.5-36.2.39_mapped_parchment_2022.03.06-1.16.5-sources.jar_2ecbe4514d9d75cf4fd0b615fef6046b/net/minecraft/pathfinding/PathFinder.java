...
public class PathFinder {
   ...
   public Path findPath(Region pRegion, MobEntity pMob, Set<BlockPos> pTargetPositions, float pMaxRange, int pAccuracy, float pSearchDepthMultiplier) {
      ...
      Map<FlaggedPathPoint, BlockPos> map = pTargetPositions.stream().collect(Collectors.toMap((p_224782_1_) -> {
         return this.nodeEvaluator.getGoal((double)p_224782_1_.getX(), (double)p_224782_1_.getY(), (double)p_224782_1_.getZ());
      }, Function.identity()));
      Path path = this.findPath(pathpoint, map, pMaxRange, pAccuracy, pSearchDepthMultiplier);
    ...
    */
   private Path reconstructPath(PathPoint pPoint, BlockPos pTargetPos, boolean pReachesTarget) {
      List<PathPoint> list = Lists.newArrayList();
      PathPoint pathpoint = pPoint;
      list.add(0, pPoint);
