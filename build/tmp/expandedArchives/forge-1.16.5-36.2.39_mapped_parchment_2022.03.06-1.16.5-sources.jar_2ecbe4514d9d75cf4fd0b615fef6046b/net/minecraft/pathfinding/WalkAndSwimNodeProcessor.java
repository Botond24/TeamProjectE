...
public class WalkAndSwimNodeProcessor extends WalkNodeProcessor {
   ...
   protected PathNodeType evaluateBlockPathType(IBlockReader pLevel, boolean pCanOpenDoors, boolean pCanEnterDoors, BlockPos pPos, PathNodeType pNodeType) {
      ...
      if (pNodeType == PathNodeType.DOOR_OPEN || pNodeType == PathNodeType.DOOR_WOOD_CLOSED || pNodeType == PathNodeType.DOOR_IRON_CLOSED) {
   ...
   public PathNodeType getBlockPathType(IBlockReader pLevel, int pX, int pY, int pZ) {
      ...
      if (pathnodetype == PathNodeType.WATER) {
         for(Direction direction : Direction.values()) {
            PathNodeType pathnodetype2 = getBlockPathTypeRaw(pLevel, blockpos$mutable.set(pX, pY, pZ).move(direction));
         ...
         if (pathnodetype == PathNodeType.OPEN && pY >= 1) {
            BlockState blockstate = pLevel.getBlockState(new BlockPos(pX, pY - 1, pZ));
