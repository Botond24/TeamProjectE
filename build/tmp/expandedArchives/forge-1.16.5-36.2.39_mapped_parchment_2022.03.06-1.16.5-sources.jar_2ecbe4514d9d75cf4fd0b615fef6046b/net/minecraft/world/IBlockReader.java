...
public interface IBlockReader {
   @Nullable
   TileEntity getBlockEntity(BlockPos pPos);
   ...
   default int getLightEmission(BlockPos pPos) {
      return this.getBlockState(pPos).getLightValue(this, pPos);
   }
   ...
   default Stream<BlockState> getBlockStates(AxisAlignedBB pArea) {
      return BlockPos.betweenClosedStream(pArea).map(this::getBlockState);
   }

   default BlockRayTraceResult clip(RayTraceContext pContext) {
      return traverseBlocks(pContext, (p_217297_1_, p_217297_2_) -> {
   ...
   @Nullable
   default BlockRayTraceResult clipWithInteractionOverride(Vector3d pStartVec, Vector3d pEndVec, BlockPos pPos, VoxelShape pShape, BlockState pState) {
      BlockRayTraceResult blockraytraceresult = pShape.clip(pStartVec, pEndVec, pPos);
      if (blockraytraceresult != null) {
         BlockRayTraceResult blockraytraceresult1 = pState.getInteractionShape(this, pPos).clip(pStartVec, pEndVec, pPos);
         if (blockraytraceresult1 != null && blockraytraceresult1.getLocation().subtract(pStartVec).lengthSqr() < blockraytraceresult.getLocation().subtract(pStartVec).lengthSqr()) {
   ...
   default double getBlockFloorHeight(VoxelShape pShape, Supplier<VoxelShape> p_242402_2_) {
      if (!pShape.isEmpty()) {
