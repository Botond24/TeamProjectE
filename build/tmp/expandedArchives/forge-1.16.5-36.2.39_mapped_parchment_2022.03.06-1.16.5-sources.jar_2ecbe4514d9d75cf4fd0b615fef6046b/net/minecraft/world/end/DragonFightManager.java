...
public class DragonFightManager {
   ...
   public DragonFightManager(ServerWorld pLevel, long pSeed, CompoundNBT pTag) {
      this.level = pLevel;
      if (pTag.contains("DragonKilled", 99)) {
         ...
         if (pTag.contains("ExitPortalLocation", 10)) {
            this.portalLocation = NBTUtil.readBlockPos(pTag.getCompound("ExitPortalLocation"));
         }
      } else {
      ...
      if (pTag.contains("Gateways", 9)) {
         ListNBT listnbt = pTag.getList("Gateways", 3);
         ...
         Collections.shuffle(this.gateways, new Random(pSeed));
      }
   ...
   protected void setRespawnStage(DragonSpawnState pState) {
      if (this.respawnStage == null) {
         throw new IllegalStateException("Dragon respawn isn't in progress, can't skip ahead in the animation.");
      } else {
         this.respawnTime = 0;
         if (pState == DragonSpawnState.END) {
            this.respawnStage = null;
            this.dragonKilled = false;
   ...
   public void setDragonKilled(EnderDragonEntity pDragon) {
   ...
   public void updateDragon(EnderDragonEntity pDragon) {
      if (pDragon.getUUID().equals(this.dragonUUID)) {
         this.dragonEvent.setPercent(pDragon.getHealth() / pDragon.getMaxHealth());
         this.ticksSinceDragonSeen = 0;
   ...
   public void onCrystalDestroyed(EnderCrystalEntity pCrystal, DamageSource pDmgSrc) {
      if (this.respawnStage != null && this.respawnCrystals.contains(pCrystal)) {
         ...
         if (entity instanceof EnderDragonEntity) {
            ((EnderDragonEntity)entity).onCrystalDestroyed(pCrystal, pCrystal.blockPosition(), pDmgSrc);
         }
      }
   ...
   private void respawnDragon(List<EnderCrystalEntity> pCrystals) {
      if (this.dragonKilled && this.respawnStage == null) {
