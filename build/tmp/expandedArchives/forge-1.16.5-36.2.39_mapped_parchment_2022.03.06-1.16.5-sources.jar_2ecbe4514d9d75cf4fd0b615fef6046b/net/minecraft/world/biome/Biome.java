...
public final class Biome extends net.minecraftforge.registries.ForgeRegistryEntry.UncheckedRegistryEntry<Biome> {
   ...
   public boolean shouldFreeze(IWorldReader pLevel, BlockPos pPos) {
      return this.shouldFreeze(pLevel, pPos, true);
   }

   public boolean shouldFreeze(IWorldReader pLevel, BlockPos pWater, boolean pMustBeAtEdge) {
      if (this.getTemperature(pWater) >= 0.15F) {
         return false;
      } else {
         if (pWater.getY() >= 0 && pWater.getY() < 256 && pLevel.getBrightness(LightType.BLOCK, pWater) < 10) {
   ...
   public boolean shouldSnow(IWorldReader pLevel, BlockPos pPos) {
      if (this.getTemperature(pPos) >= 0.15F) {
         return false;
      } else {
         if (pPos.getY() >= 0 && pPos.getY() < 256 && pLevel.getBrightness(LightType.BLOCK, pPos) < 10) {
            BlockState blockstate = pLevel.getBlockState(pPos);
            if (blockstate.isAir(pLevel, pPos) && Blocks.SNOW.defaultBlockState().canSurvive(pLevel, pPos)) {
               return true;
            }
         }
   ...
   public void generate(StructureManager pStructureManager, ChunkGenerator pChunkGenerator, WorldGenRegion pLevelGenRegion, long pSeed, SharedSeedRandom pRandom, BlockPos pPos) {
      ...
      for(int j = 0; j < i; ++j) {
         ...
         if (pStructureManager.shouldGenerateFeatures()) {
            for(Structure<?> structure : this.structuresByStep.getOrDefault(j, Collections.emptyList())) {
               pRandom.setFeatureSeed(pSeed, k, j);
               int l = pPos.getX() >> 4;
               int i1 = pPos.getZ() >> 4;
               int j1 = l << 4;
               int k1 = i1 << 4;
               ...
               try {
                  pStructureManager.startsForFeature(SectionPos.of(pPos), structure).forEach((p_242426_8_) -> {
                     p_242426_8_.placeInChunk(pLevelGenRegion, pStructureManager, pChunkGenerator, pRandom, new MutableBoundingBox(j1, k1, j1 + 15, k1 + 15), new ChunkPos(l, i1));
                  });
         ...
         if (list.size() > j) {
            for(Supplier<ConfiguredFeature<?, ?>> supplier : list.get(j)) {
               ...
               try {
                  configuredfeature.place(pLevelGenRegion, pChunkGenerator, pRandom, pPos);
   ...
   public static class Attributes {
      ...
      public float fitness(Biome.Attributes pAttributes) {
   ...
   public static class Builder {
      ...
      public Biome.Builder precipitation(Biome.RainType pPrecipitation) {
         this.precipitation = pPrecipitation;
         return this;
      }

      public Biome.Builder biomeCategory(Biome.Category pBiomeCategory) {
         this.biomeCategory = pBiomeCategory;
         return this;
      }

      public Biome.Builder depth(float pDepth) {
         this.depth = pDepth;
         return this;
      }

      public Biome.Builder scale(float pScale) {
         this.scale = pScale;
         return this;
      }

      public Biome.Builder temperature(float pTemperature) {
         this.temperature = pTemperature;
         return this;
      }

      public Biome.Builder downfall(float pDownfall) {
         this.downfall = pDownfall;
         return this;
      }

      public Biome.Builder specialEffects(BiomeAmbience pEffects) {
         this.specialEffects = pEffects;
         return this;
      }

      public Biome.Builder mobSpawnSettings(MobSpawnInfo pMobSpawnSettings) {
         this.mobSpawnSettings = pMobSpawnSettings;
         return this;
      }

      public Biome.Builder generationSettings(BiomeGenerationSettings pGenerationSettings) {
         this.generationSettings = pGenerationSettings;
         return this;
      }

      public Biome.Builder temperatureAdjustment(Biome.TemperatureModifier pTemperatureSettings) {
   ...
   public static enum TemperatureModifier implements IStringSerializable {
      ...
      private TemperatureModifier(String pName) {
         this.name = pName;
      }
