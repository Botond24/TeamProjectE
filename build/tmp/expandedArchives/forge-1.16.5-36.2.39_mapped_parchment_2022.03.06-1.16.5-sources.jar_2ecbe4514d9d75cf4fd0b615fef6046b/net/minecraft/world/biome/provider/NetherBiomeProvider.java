...
public class NetherBiomeProvider extends BiomeProvider {
   ...
   private NetherBiomeProvider(long pSeed, List<Pair<Biome.Attributes, Supplier<Biome>>> pParameters, Optional<Pair<Registry<Biome>, NetherBiomeProvider.Preset>> pPreset) {
   ...
   private NetherBiomeProvider(long pSeed, List<Pair<Biome.Attributes, Supplier<Biome>>> pParameters, NetherBiomeProvider.Noise pTemperatureParams, NetherBiomeProvider.Noise pHumidityParams, NetherBiomeProvider.Noise pAltitudeParams, NetherBiomeProvider.Noise pWeirdnessParams, Optional<Pair<Registry<Biome>, NetherBiomeProvider.Preset>> pPreset) {
      ...
      this.temperatureNoise = MaxMinNoiseMixer.create(new SharedSeedRandom(pSeed), pTemperatureParams.firstOctave(), pTemperatureParams.amplitudes());
      this.humidityNoise = MaxMinNoiseMixer.create(new SharedSeedRandom(pSeed + 1L), pHumidityParams.firstOctave(), pHumidityParams.amplitudes());
      this.altitudeNoise = MaxMinNoiseMixer.create(new SharedSeedRandom(pSeed + 2L), pAltitudeParams.firstOctave(), pAltitudeParams.amplitudes());
      this.weirdnessNoise = MaxMinNoiseMixer.create(new SharedSeedRandom(pSeed + 3L), pWeirdnessParams.firstOctave(), pWeirdnessParams.amplitudes());
   ...
   public BiomeProvider withSeed(long pSeed) {
      return new NetherBiomeProvider(pSeed, this.parameters, this.temperatureParams, this.humidityParams, this.altitudeParams, this.weirdnessParams, this.preset);
   }
   ...
   public boolean stable(long pSeed) {
      return this.seed == pSeed && this.preset.isPresent() && Objects.equals(this.preset.get().getSecond(), NetherBiomeProvider.Preset.NETHER);
   }
   ...
   public static class Preset {
      ...
      public Preset(ResourceLocation pName, Function3<NetherBiomeProvider.Preset, Registry<Biome>, Long, NetherBiomeProvider> pBiomeSource) {
         this.name = pName;
         this.biomeSource = pBiomeSource;
         BY_NAME.put(pName, this);
      }

      public NetherBiomeProvider biomeSource(Registry<Biome> pLookupRegistry, long pSeed) {
