...
public class ChunkSerializer {
   ...
   public static ChunkPrimer read(ServerWorld pLevel, TemplateManager pTemplateManager, PointOfInterestManager pPoiManager, ChunkPos pPos, CompoundNBT pCompound) {
      ...
      CompoundNBT compoundnbt = pCompound.getCompound("Level");
      ...
      BiomeContainer biomecontainer = new BiomeContainer(pLevel.registryAccess().registryOrThrow(Registry.BIOME_REGISTRY), pPos, biomeprovider, compoundnbt.contains("Biomes", 11) ? compoundnbt.getIntArray("Biomes") : null);
      ...
      }, pPos, compoundnbt.getList("ToBeTicked", 9));
      ...
      }, pPos, compoundnbt.getList("LiquidsToBeTicked", 9));
      ...
      if (flag) {
         worldlightmanager.retainData(pPos, true);
      }
      ...
      for(int j = 0; j < listnbt.size(); ++j) {
         ...
         if (compoundnbt1.contains("Palette", 9) && compoundnbt1.contains("BlockStates", 12)) {
            ...
            pPoiManager.checkConsistencyWithBlocks(pPos, chunksection);
         }

         if (flag) {
            if (compoundnbt1.contains("BlockLight", 7)) {
               worldlightmanager.queueSectionData(LightType.BLOCK, SectionPos.of(pPos, k), new NibbleArray(compoundnbt1.getByteArray("BlockLight")), true);
            }

            if (flag1 && compoundnbt1.contains("SkyLight", 7)) {
               worldlightmanager.queueSectionData(LightType.SKY, SectionPos.of(pPos, k), new NibbleArray(compoundnbt1.getByteArray("SkyLight")), true);
            }
         }
      }

      long k1 = compoundnbt.getLong("InhabitedTime");
      ChunkStatus.Type chunkstatus$type = getChunkTypeFromTag(pCompound);
      IChunk ichunk;
      if (chunkstatus$type == ChunkStatus.Type.LEVELCHUNK) {
         ...
         ichunk = new Chunk(pLevel.getLevel(), pPos, biomecontainer, upgradedata, iticklist, iticklist1, k1, achunksection, (p_222648_1_) -> {
         ...
         ChunkPrimer chunkprimer = new ChunkPrimer(pPos, upgradedata, achunksection, chunkprimerticklist, chunkprimerticklist1);
         ...
         if (!flag && chunkprimer.getStatus().isOrAfter(ChunkStatus.LIGHT)) {
            for(BlockPos blockpos : BlockPos.betweenClosed(pPos.getMinBlockX(), 0, pPos.getMinBlockZ(), pPos.getMaxBlockX(), 255, pPos.getMaxBlockZ())) {
      ...
      ichunk.setAllStarts(unpackStructureStart(pTemplateManager, compoundnbt4, pLevel.getSeed()));
      net.minecraftforge.common.ForgeHooks.fixNullStructureReferences(ichunk, unpackStructureReferences(pPos, compoundnbt4));
      ...
      if (chunkstatus$type == ChunkStatus.Type.LEVELCHUNK) {
         net.minecraftforge.common.MinecraftForge.EVENT_BUS.post(new net.minecraftforge.event.world.ChunkDataEvent.Load(ichunk, pCompound, chunkstatus$type));
   ...
   public static CompoundNBT write(ServerWorld pLevel, IChunk pChunk) {
      ...
      compoundnbt1.putLong("LastUpdate", pLevel.getGameTime());
      compoundnbt1.putLong("InhabitedTime", pChunk.getInhabitedTime());
      compoundnbt1.putString("Status", pChunk.getStatus().getName());
      UpgradeData upgradedata = pChunk.getUpgradeData();
      if (!upgradedata.isEmpty()) {
         compoundnbt1.put("UpgradeData", upgradedata.write());
      }

      ChunkSection[] achunksection = pChunk.getSections();
      ListNBT listnbt = new ListNBT();
      WorldLightManager worldlightmanager = pLevel.getChunkSource().getLightEngine();
      ...
      BiomeContainer biomecontainer = pChunk.getBiomes();
      ...
      for(BlockPos blockpos : pChunk.getBlockEntitiesPos()) {
         CompoundNBT compoundnbt4 = pChunk.getBlockEntityNbtForSaving(blockpos);
      ...
      if (pChunk.getStatus().getChunkType() == ChunkStatus.Type.LEVELCHUNK) {
         ...
         ChunkPrimer chunkprimer = (ChunkPrimer)pChunk;
      ...
      ITickList<Block> iticklist = pChunk.getBlockTicks();
      if (iticklist instanceof ChunkPrimerTickList) {
         ...
         compoundnbt1.put("TileTicks", pLevel.getBlockTicks().save(chunkpos));
      }

      ITickList<Fluid> iticklist1 = pChunk.getLiquidTicks();
      if (iticklist1 instanceof ChunkPrimerTickList) {
         ...
         compoundnbt1.put("LiquidTicks", pLevel.getLiquidTicks().save(chunkpos));
      }

      compoundnbt1.put("PostProcessing", packOffsets(pChunk.getPostProcessing()));
      CompoundNBT compoundnbt6 = new CompoundNBT();

      for(Entry<Heightmap.Type, Heightmap> entry : pChunk.getHeightmaps()) {
      ...
      compoundnbt1.put("Structures", packStructureData(chunkpos, pChunk.getAllStarts(), pChunk.getAllReferences()));
      return compoundnbt;
   }

   public static ChunkStatus.Type getChunkTypeFromTag(@Nullable CompoundNBT pChunkNBT) {
      if (pChunkNBT != null) {
   ...
   private static Map<Structure<?>, LongSet> unpackStructureReferences(ChunkPos pPos, CompoundNBT pTag) {
      Map<Structure<?>, LongSet> map = Maps.newHashMap();
      CompoundNBT compoundnbt = pTag.getCompound("References");
      ...
      for(String s : compoundnbt.getAllKeys()) {
         map.put(Structure.STRUCTURES_REGISTRY.get(s.toLowerCase(Locale.ROOT)), new LongOpenHashSet(Arrays.stream(compoundnbt.getLongArray(s)).filter((p_227074_2_) -> {
            ...
            if (chunkpos.getChessboardDistance(pPos) > 8) {
               LOGGER.warn("Found invalid structure reference [ {} @ {} ] for chunk {}.", s, chunkpos, pPos);
               return false;
            } else {
               return true;
            }
   ...
   public static ListNBT packOffsets(ShortList[] pList) {
      ListNBT listnbt = new ListNBT();
