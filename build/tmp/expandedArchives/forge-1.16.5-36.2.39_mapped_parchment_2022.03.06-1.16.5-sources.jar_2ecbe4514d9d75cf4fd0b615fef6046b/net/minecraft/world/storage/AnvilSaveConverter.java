...
public class AnvilSaveConverter {
   ...
   static boolean convertLevel(SaveFormat.LevelSave pLevelSave, IProgressUpdate pProgress) {
      pProgress.progressStagePercentage(0);
      List<File> list = Lists.newArrayList();
      List<File> list1 = Lists.newArrayList();
      List<File> list2 = Lists.newArrayList();
      File file1 = pLevelSave.getDimensionPath(World.OVERWORLD);
      File file2 = pLevelSave.getDimensionPath(World.NETHER);
      File file3 = pLevelSave.getDimensionPath(World.END);
      ...
      IServerConfiguration iserverconfiguration = pLevelSave.getDataTag(worldsettingsimport, DatapackCodec.DEFAULT);
      ...
      convertRegions(dynamicregistries$impl, new File(file1, "region"), list, biomeprovider, 0, i, pProgress);
      convertRegions(dynamicregistries$impl, new File(file2, "region"), list1, new SingleBiomeProvider(registry.getOrThrow(Biomes.NETHER_WASTES)), list.size(), i, pProgress);
      convertRegions(dynamicregistries$impl, new File(file3, "region"), list2, new SingleBiomeProvider(registry.getOrThrow(Biomes.THE_END)), list.size() + list1.size(), i, pProgress);
      makeMcrLevelDatBackup(pLevelSave);
      pLevelSave.saveDataTag(dynamicregistries$impl, iserverconfiguration);
      return true;
   }

   private static void makeMcrLevelDatBackup(SaveFormat.LevelSave pLevelSave) {
      File file1 = pLevelSave.getLevelPath(FolderName.LEVEL_DATA_FILE).toFile();
      if (!file1.exists()) {
