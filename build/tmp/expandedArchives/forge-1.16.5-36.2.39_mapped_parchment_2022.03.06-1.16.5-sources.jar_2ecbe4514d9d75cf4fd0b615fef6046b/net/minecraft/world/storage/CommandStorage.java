...
public class CommandStorage {
   ...
   public CompoundNBT get(ResourceLocation pId) {
      ...
      return commandstorage$container != null ? commandstorage$container.get(pId.getPath()) : new CompoundNBT();
   }

   public void set(ResourceLocation pId, CompoundNBT pNbt) {
      String s = pId.getNamespace();
      String s1 = createId(s);
      this.storage.computeIfAbsent(() -> {
         return this.newStorage(s, s1);
      }, s1).put(pId.getPath(), pNbt);
   }
   ...
   static class Container extends WorldSavedData {
      ...
      public CompoundNBT save(CompoundNBT pCompound) {
         ...
         pCompound.put("contents", compoundnbt);
         return pCompound;
      }

      public CompoundNBT get(String pId) {
         CompoundNBT compoundnbt = this.storage.get(pId);
         return compoundnbt != null ? compoundnbt : new CompoundNBT();
      }

      public void put(String pId, CompoundNBT pNbt) {
         if (pNbt.isEmpty()) {
      ...
      public Stream<ResourceLocation> getKeys(String pNamespace) {
         return this.storage.keySet().stream().map((p_227494_1_) -> {
            return new ResourceLocation(pNamespace, p_227494_1_);
         });
      }
   }
}