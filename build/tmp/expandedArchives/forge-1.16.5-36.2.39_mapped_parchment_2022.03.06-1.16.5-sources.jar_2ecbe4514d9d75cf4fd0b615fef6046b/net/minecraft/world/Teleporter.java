...
public class Teleporter implements net.minecraftforge.common.util.ITeleporter {
   ...
   public Optional<TeleportationRepositioner.Result> findPortalAround(BlockPos pPos, boolean pIsNether) {
      ...
      }, pPos, i, PointOfInterestManager.Status.ANY).sorted(Comparator.<PointOfInterest>comparingDouble((p_242954_1_) -> {
   ...
   public Optional<TeleportationRepositioner.Result> createPortal(BlockPos pPos, Direction.Axis pAxis) {
      ...
      for(BlockPos.Mutable blockpos$mutable1 : BlockPos.spiralAround(pPos, 16, Direction.EAST, Direction.SOUTH)) {
   ...
   private boolean canHostFrame(BlockPos pOriginalPos, BlockPos.Mutable pOffsetPos, Direction pDirection, int pOffsetScale) {
