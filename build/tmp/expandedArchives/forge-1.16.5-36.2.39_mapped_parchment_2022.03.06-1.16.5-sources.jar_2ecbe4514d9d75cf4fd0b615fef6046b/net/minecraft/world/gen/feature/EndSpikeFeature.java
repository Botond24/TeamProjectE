...
public class EndSpikeFeature extends Feature<EndSpikeFeatureConfig> {
   ...
   public static List<EndSpikeFeature.EndSpike> getSpikesForLevel(ISeedReader pLevel) {
    ...
    */
   private void placeSpike(IServerWorld pLevel, Random pRandom, EndSpikeFeatureConfig pConfig, EndSpikeFeature.EndSpike pSpike) {
      int i = pSpike.getRadius();

      for(BlockPos blockpos : BlockPos.betweenClosed(new BlockPos(pSpike.getCenterX() - i, 0, pSpike.getCenterZ() - i), new BlockPos(pSpike.getCenterX() + i, pSpike.getHeight() + 10, pSpike.getCenterZ() + i))) {
            ...
            for(int l = -2; l <= 2; ++l) {
               for(int i1 = 0; i1 <= 3; ++i1) {
                  ...
                  if (flag || flag1 || flag2) {
                     ...
                     this.setBlock(pLevel, blockpos$mutable.set(pSpike.getCenterX() + k, pSpike.getHeight() + i1, pSpike.getCenterZ() + l), blockstate);
                  }
               }
            }
         }
      ...
      EnderCrystalEntity endercrystalentity = EntityType.END_CRYSTAL.create(pLevel.getLevel());
      endercrystalentity.setBeamTarget(pConfig.getCrystalBeamTarget());
      endercrystalentity.setInvulnerable(pConfig.isCrystalInvulnerable());
      endercrystalentity.moveTo((double)pSpike.getCenterX() + 0.5D, (double)(pSpike.getHeight() + 1), (double)pSpike.getCenterZ() + 0.5D, pRandom.nextFloat() * 360.0F, 0.0F);
      pLevel.addFreshEntity(endercrystalentity);
   ...
   public static class EndSpike {
      ...
      public boolean isCenterWithinChunk(BlockPos pPos) {
         return pPos.getX() >> 4 == this.centerX >> 4 && pPos.getZ() >> 4 == this.centerZ >> 4;
      }
