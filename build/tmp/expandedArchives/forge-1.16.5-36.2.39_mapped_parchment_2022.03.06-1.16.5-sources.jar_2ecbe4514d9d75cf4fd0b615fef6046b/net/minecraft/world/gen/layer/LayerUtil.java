...
public class LayerUtil {
   ...
   public static <T extends IArea, C extends IExtendedNoiseRandom<T>> IAreaFactory<T> zoom(long pSeed, IAreaTransformer1 pParent, IAreaFactory<T> p_202829_3_, int pCount, LongFunction<C> pContextFactory) {
      IAreaFactory<T> iareafactory = p_202829_3_;

      for(int i = 0; i < pCount; ++i) {
         iareafactory = pParent.run(pContextFactory.apply(pSeed + (long)i), iareafactory);
      }
