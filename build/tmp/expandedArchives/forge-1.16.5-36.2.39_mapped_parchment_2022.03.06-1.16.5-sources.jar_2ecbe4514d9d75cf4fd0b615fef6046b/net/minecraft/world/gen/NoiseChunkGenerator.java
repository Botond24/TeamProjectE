...
public final class NoiseChunkGenerator extends ChunkGenerator {
   public static final Codec<NoiseChunkGenerator> CODEC = RecordCodecBuilder.create((p_236091_0_) -> {
   ...
   private NoiseChunkGenerator(BiomeProvider pBiomeSource, BiomeProvider pRuntimeBiomeSource, long pSeed, Supplier<DimensionSettings> pSettings) {
      super(pBiomeSource, pRuntimeBiomeSource, pSettings.get().structureSettings(), pSeed);
      this.seed = pSeed;
      DimensionSettings dimensionsettings = pSettings.get();
      ...
      this.random = new SharedSeedRandom(pSeed);
   ...
   public ChunkGenerator withSeed(long pSeed) {
      return new NoiseChunkGenerator(this.biomeSource.withSeed(pSeed), pSeed, this.settings);
   }

   public boolean stable(long pSeed, RegistryKey<DimensionSettings> pSettingsKey) {
   ...
   public void buildSurfaceAndBedrock(WorldGenRegion pLevel, IChunk pChunk) {
      ...
      for(int i1 = 0; i1 < 16; ++i1) {
         for(int j1 = 0; j1 < 16; ++j1) {
            int k1 = k + i1;
            int l1 = l + j1;
            int i2 = pChunk.getHeight(Heightmap.Type.WORLD_SURFACE_WG, i1, j1) + 1;
            double d1 = this.surfaceNoise.getSurfaceNoiseValue((double)k1 * 0.0625D, (double)l1 * 0.0625D, 0.0625D, (double)i1 * 0.0625D) * 15.0D;
            pLevel.getBiome(blockpos$mutable.set(k + i1, i2, l + j1)).buildSurfaceAt(sharedseedrandom, pChunk, k1, l1, i2, d1, this.defaultBlock, this.defaultFluid, this.getSeaLevel(), pLevel.getSeed());
         }
      }

      this.setBedrock(pChunk, sharedseedrandom);
   }

   private void setBedrock(IChunk pChunk, Random pRandom) {
      BlockPos.Mutable blockpos$mutable = new BlockPos.Mutable();
         ...
         for(BlockPos blockpos : BlockPos.betweenClosed(i, 0, j, i + 15, 0, j + 15)) {
            if (flag) {
               for(int j1 = 0; j1 < 5; ++j1) {
                  if (j1 <= pRandom.nextInt(5)) {
                     pChunk.setBlockState(blockpos$mutable.set(blockpos.getX(), l - j1, blockpos.getZ()), Blocks.BEDROCK.defaultBlockState(), false);
                  }
               }
            }
            ...
            if (flag1) {
               for(int k1 = 4; k1 >= 0; --k1) {
                  if (k1 <= pRandom.nextInt(5)) {
                     pChunk.setBlockState(blockpos$mutable.set(blockpos.getX(), k + k1, blockpos.getZ()), Blocks.BEDROCK.defaultBlockState(), false);
                  }
               }
            }
         }
   ...
   public void spawnOriginalMobs(WorldGenRegion pRegion) {
      if (!this.settings.get().disableMobGeneration()) {
         ...
         Biome biome = pRegion.getBiome((new ChunkPos(i, j)).getWorldPosition());
         SharedSeedRandom sharedseedrandom = new SharedSeedRandom();
         sharedseedrandom.setDecorationSeed(pRegion.getSeed(), i << 4, j << 4);
         WorldEntitySpawner.spawnMobsForChunkGeneration(pRegion, biome, i, j, sharedseedrandom);
      }
   }
}
