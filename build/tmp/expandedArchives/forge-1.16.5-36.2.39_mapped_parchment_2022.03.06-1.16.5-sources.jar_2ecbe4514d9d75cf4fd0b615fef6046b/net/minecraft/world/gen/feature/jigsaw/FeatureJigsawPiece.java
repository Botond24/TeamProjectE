...
public class FeatureJigsawPiece extends JigsawPiece {
   ...
   public List<Template.BlockInfo> getShuffledJigsawBlocks(TemplateManager pTemplateManager, BlockPos pPos, Rotation pRotation, Random pRandom) {
      ...
      list.add(new Template.BlockInfo(pPos, Blocks.JIGSAW.defaultBlockState().setValue(JigsawBlock.ORIENTATION, JigsawOrientation.fromFrontAndTop(Direction.DOWN, Direction.SOUTH)), this.defaultJigsawNBT));
      return list;
   }

   public MutableBoundingBox getBoundingBox(TemplateManager pTemplateManager, BlockPos pPos, Rotation pRotation) {
      BlockPos blockpos = this.getSize(pTemplateManager, pRotation);
      return new MutableBoundingBox(pPos.getX(), pPos.getY(), pPos.getZ(), pPos.getX() + blockpos.getX(), pPos.getY() + blockpos.getY(), pPos.getZ() + blockpos.getZ());
   }
