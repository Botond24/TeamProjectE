...
public abstract class Structure<C extends IFeatureConfig> extends net.minecraftforge.registries.ForgeRegistryEntry<Structure<?>> implements net.minecraftforge.common.extensions.IForgeStructure {
   ...
   private static <F extends Structure<?>> F register(String pName, F pStructure, GenerationStage.Decoration pDecorationStage) {
   ...
   public Structure(Codec<C> pCodec) {
      this.configuredStructureCodec = pCodec.fieldOf("config").xmap((p_236395_1_) -> {
   ...
   public StructureFeature<C, ? extends Structure<C>> configured(C pConfig) {
      return new StructureFeature<>(this, pConfig);
   }

   @Nullable
   public BlockPos getNearestGeneratedFeature(IWorldReader pLevel, StructureManager pStructureManager, BlockPos pPos, int pSearchRadius, boolean pSkipKnownStructures, long pSeed, StructureSeparationSettings pConfig) {
      int i = pConfig.spacing();
      int j = pPos.getX() >> 4;
      int k = pPos.getZ() >> 4;
      int l = 0;

      for(SharedSeedRandom sharedseedrandom = new SharedSeedRandom(); l <= pSearchRadius; ++l) {
         for(int i1 = -l; i1 <= l; ++i1) {
            ...
            for(int j1 = -l; j1 <= l; ++j1) {
               ...
               if (flag || flag1) {
                  int k1 = j + i * i1;
                  int l1 = k + i * j1;
                  ChunkPos chunkpos = this.getPotentialFeatureChunk(pConfig, pSeed, sharedseedrandom, k1, l1);
                  IChunk ichunk = pLevel.getChunk(chunkpos.x, chunkpos.z, ChunkStatus.STRUCTURE_STARTS);
                  StructureStart<?> structurestart = pStructureManager.getStartForFeature(SectionPos.of(ichunk.getPos(), 0), this, ichunk);
   ...
   public final ChunkPos getPotentialFeatureChunk(StructureSeparationSettings pSeparationSettings, long pSeed, SharedSeedRandom pRandom, int pX, int pZ) {
