...
public class SimplexNoiseGenerator {
   ...
   protected static double dot(int[] pGradElement, double pXFactor, double pYFactor, double pZFactor) {
      return (double)pGradElement[0] * pXFactor + (double)pGradElement[1] * pYFactor + (double)pGradElement[2] * pZFactor;
   }

   private double getCornerNoise3D(int pGradIndex, double pX, double pY, double pZ, double pOffset) {
      ...
      if (d1 < 0.0D) {
         ...
         d1 = d1 * d1;
         d0 = d1 * d1 * dot(GRADIENT[pGradIndex], pX, pY, pZ);
      }
   ...
   public double getValue(double pX, double pY) {
      double d0 = (pX + pY) * F2;
      int i = MathHelper.floor(pX + d0);
      int j = MathHelper.floor(pY + d0);
   ...
   public double getValue(double pX, double pY, double pZ) {
      ...
      int i = MathHelper.floor(pX + d1);
      int j = MathHelper.floor(pY + d1);
      int k = MathHelper.floor(pZ + d1);
