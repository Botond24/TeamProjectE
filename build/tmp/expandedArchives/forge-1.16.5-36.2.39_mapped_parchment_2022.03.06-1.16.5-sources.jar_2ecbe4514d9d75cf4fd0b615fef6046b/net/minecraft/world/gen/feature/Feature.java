...
public abstract class Feature<FC extends IFeatureConfig> extends net.minecraftforge.registries.ForgeRegistryEntry<Feature<?>> {
   ...
   private static <C extends IFeatureConfig, F extends Feature<C>> F register(String pKey, F pValue) {
      return Registry.register(Registry.FEATURE, pKey, pValue);
   }

   public Feature(Codec<FC> pCodec) {
      this.configuredCodec = pCodec.fieldOf("config").xmap((p_236296_1_) -> {
   ...
   public static boolean isGrassOrDirt(IWorldGenerationBaseReader pLevel, BlockPos pPos) {
      return pLevel.isStateAtPosition(pPos, (p_236294_0_) -> {
         return isDirt(p_236294_0_.getBlock());
      });
   }

   public static boolean isAir(IWorldGenerationBaseReader pLevel, BlockPos pPos) {
      return pLevel.isStateAtPosition(pPos, AbstractBlock.AbstractBlockState::isAir);
   }
}
