...
public abstract class ValleySurfaceBuilder extends SurfaceBuilder<SurfaceBuilderConfig> {
   ...
   public void initNoise(long pSeed) {
      if (this.seed != pSeed || this.patchNoise == null || this.floorNoises.isEmpty() || this.ceilingNoises.isEmpty()) {
         ...
         this.patchNoise = new OctavesNoiseGenerator(new SharedSeedRandom(pSeed + (long)this.floorNoises.size() + (long)this.ceilingNoises.size()), ImmutableList.of(0));
      }

      this.seed = pSeed;
   }

   private static ImmutableMap<BlockState, OctavesNoiseGenerator> initPerlinNoises(ImmutableList<BlockState> pStates, long pSeed) {
      Builder<BlockState, OctavesNoiseGenerator> builder = new Builder<>();

      for(BlockState blockstate : pStates) {
         builder.put(blockstate, new OctavesNoiseGenerator(new SharedSeedRandom(pSeed), ImmutableList.of(-4)));
         ++pSeed;
      }
