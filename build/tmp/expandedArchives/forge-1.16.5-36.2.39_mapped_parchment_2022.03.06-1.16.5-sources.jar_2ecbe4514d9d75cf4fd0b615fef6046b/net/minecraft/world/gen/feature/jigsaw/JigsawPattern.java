...
public class JigsawPattern {
   ...
   public JigsawPattern(ResourceLocation pName, ResourceLocation pFallback, List<Pair<Function<JigsawPattern.PlacementBehaviour, ? extends JigsawPiece>, Integer>> p_i51397_3_, JigsawPattern.PlacementBehaviour p_i51397_4_) {
      this.name = pName;
   ...
   public int getMaxSize(TemplateManager pTemplateManager) {
      if (this.maxSize == Integer.MIN_VALUE) {
         this.maxSize = this.templates.stream().mapToInt((p_236856_1_) -> {
            return p_236856_1_.getBoundingBox(pTemplateManager, BlockPos.ZERO, Rotation.NONE).getYSpan();
         }).max().orElse(0);
      }
   ...
   public JigsawPiece getRandomTemplate(Random pRandom) {
      return this.templates.get(pRandom.nextInt(this.templates.size()));
   }

   public List<JigsawPiece> getShuffledTemplates(Random pRandom) {
      return ImmutableList.copyOf(ObjectArrays.shuffle(this.templates.toArray(new JigsawPiece[0]), pRandom));
   }
   ...
   public static enum PlacementBehaviour implements IStringSerializable, net.minecraftforge.common.IExtensibleEnum {
      ...
      private PlacementBehaviour(String pName, ImmutableList<StructureProcessor> pProcessors) {
         this.name = pName;
      ...
      public static PlacementBehaviour create(String enumName, String pName, ImmutableList<StructureProcessor> pProcessors) {
