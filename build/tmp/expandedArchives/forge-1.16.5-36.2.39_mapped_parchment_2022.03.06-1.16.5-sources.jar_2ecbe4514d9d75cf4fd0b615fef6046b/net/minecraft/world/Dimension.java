...
public final class Dimension {
   ...
   public static SimpleRegistry<Dimension> sortMap(SimpleRegistry<Dimension> pMappedRegistry) {
      ...
      for(RegistryKey<Dimension> registrykey : BUILTIN_ORDER) {
         Dimension dimension = pMappedRegistry.get(registrykey);
         if (dimension != null) {
            simpleregistry.register(registrykey, dimension, pMappedRegistry.lifecycle(dimension));
         }
      }

      for(Entry<RegistryKey<Dimension>, Dimension> entry : pMappedRegistry.entrySet()) {
         RegistryKey<Dimension> registrykey1 = entry.getKey();
         if (!BUILTIN_ORDER.contains(registrykey1)) {
            simpleregistry.register(registrykey1, entry.getValue(), pMappedRegistry.lifecycle(entry.getValue()));
         }
      }
   ...
   public static boolean stable(long pSeed, SimpleRegistry<Dimension> pMappedRegistry) {
      List<Entry<RegistryKey<Dimension>, Dimension>> list = Lists.newArrayList(pMappedRegistry.entrySet());
      if (list.size() != BUILTIN_ORDER.size()) {
         ...
         if (entry.getKey() == OVERWORLD && entry1.getKey() == NETHER && entry2.getKey() == END) {
            if (!entry.getValue().type().equalTo(DimensionType.DEFAULT_OVERWORLD) && entry.getValue().type() != DimensionType.DEFAULT_OVERWORLD_CAVES) {
               ...
               if (!noisechunkgenerator.stable(pSeed, DimensionSettings.NETHER)) {
                  return false;
               } else if (!noisechunkgenerator1.stable(pSeed, DimensionSettings.END)) {
                  return false;
                  ...
                  if (!netherbiomeprovider.stable(pSeed)) {
                     return false;
                     ...
                     return endbiomeprovider.stable(pSeed);
                  }
               }
            } else {
               return false;
