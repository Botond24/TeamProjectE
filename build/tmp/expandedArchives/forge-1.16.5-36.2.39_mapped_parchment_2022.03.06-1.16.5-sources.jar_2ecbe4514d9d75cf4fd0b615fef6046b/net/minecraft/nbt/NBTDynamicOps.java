...
public class NBTDynamicOps implements DynamicOps<INBT> {
   ...
   private static CollectionNBT<?> createGenericList(byte pCollectionElementId, byte pNewElementId) {
      if (typesMatch(pCollectionElementId, pNewElementId, (byte)4)) {
         return new LongArrayNBT(new long[0]);
      } else if (typesMatch(pCollectionElementId, pNewElementId, (byte)1)) {
         return new ByteArrayNBT(new byte[0]);
      } else {
         return (CollectionNBT<?>)(typesMatch(pCollectionElementId, pNewElementId, (byte)3) ? new IntArrayNBT(new int[0]) : new ListNBT());
      }
   }

   /**
    * Returns {@code true} if {@code collectionElementId} if it is the same type as the {@code newCollectionElementId}
    * and the {@code newElementId} being added is either the same type as those entries in the collection or is an
    * {@link EndTag}.
    */
   ...
   private static <T extends INBT> void fillOne(CollectionNBT<T> pCollectionTag, INBT pMergingCollectionTag, INBT pElementTag) {
      if (pMergingCollectionTag instanceof CollectionNBT) {
         CollectionNBT<?> collectionnbt = (CollectionNBT)pMergingCollectionTag;
         collectionnbt.forEach((p_240616_1_) -> {
            pCollectionTag.add((T)p_240616_1_);
         });
      }

      pCollectionTag.add((T)pElementTag);
   }

   private static <T extends INBT> void fillMany(CollectionNBT<T> pCollectionTag, INBT pMergingCollectionTag, List<INBT> pElementTags) {
      if (pMergingCollectionTag instanceof CollectionNBT) {
         CollectionNBT<?> collectionnbt = (CollectionNBT)pMergingCollectionTag;
         collectionnbt.forEach((p_240614_1_) -> {
            pCollectionTag.add((T)p_240614_1_);
         });
      }

      pElementTags.forEach((p_240607_1_) -> {
         pCollectionTag.add((T)p_240607_1_);
      });
   }
